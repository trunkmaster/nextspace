...
{
  fprintf(stderr,
          "Usage: %s [--help] [--system | --session | --bus=ADDRESS | --peer=ADDRESS] "
          "[--sender=NAME] [--dest=NAME] [--type=TYPE] [--print-reply[=literal]] "
          "[--reply-timeout=MSEC] <destination object path> <message name> [contents ...]\n",
          appname);
  exit(ecode);
}
...
int main(int argc, char *argv[])
{
  ...
  for (i = 1; i < argc && name == NULL; i++) {
    ...
    if (strcmp(arg, "--system") == 0) {
      ...
      session_or_system = TRUE;
    } else if ((strstr(arg, "--bus=") == arg) || (strstr(arg, "--peer=") == arg) ||
               (strstr(arg, "--address=") == arg)) {
  ...
  if (session_or_system && (address != NULL)) {
    fprintf(stderr,
            "\"--peer\" and \"--bus\" may not be used with \"--system\" or \"--session\"\n");
    usage(1);
  }
