...
****** TODO Первый запуск панели Terminal Services
       Если определенных сервисов нет, спросить у пользователя загружать ли
       сэмплы (посмотреть в OPENSTEP).
****** TODO Кнопка "OK"
******* TODO Нажатие по клавише "Enter"
        На данный момент все работает нормально ([panel
        setDefaultButtonCell]), но не для NSTextField. Нажатие клавиши Enter
        перехватывается окном (panel), редактирование завершается, кнопка "OK"
        нажимется.
        Должно быть: сначала принимается введенное значение (markAsChanged:),
        второе нажатие на Enter должно нажимать кнопку.
******* DONE Статус
        After "OK" button pressed futher changes won't change state of "OK"
        button and panel.
***** TODO Проверить типы создания окон (shell, program)
****** TODO Do we need "Idle" window type?
       ...
       and shells.
       Idle Window это просто первое окно, на котором ничего не
       выполняется. В это окно передается выполнение команды.
...
***** TODO Implement "Add Arguments" panel
***** TODO Проверить запуск сервисов с различными параметрами
****** TODO %s
****** TODO %p
****** TODO %s %p
**** TODO Edit->Find
