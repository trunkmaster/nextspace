...
**** TODO Расстояния между цифрами
     ...
     Как тест, можно создать Time.preferences для Preferences.app.
**** TODO Добавить языков
     Сделать картинки с днями недели и месяцами для разных языков (*.lproj).
      Минимум:
        - English, Ukrainian;
      Максимум:
        - языки, которые поддерживаются в GNUstep
     В дальнейшем это можно будет использовать в Preferences.app
*** TODO NXPreferences = PrefsModule, PrefsConttroller (PrefsModule.h)
    UI for standard application preferences, API for .preferences bundles
**** TODO Make part of Preferences application
*** TODO NXAlertPanel
...
*** DONE NXFileManager - customized methods for manipulating files/dirs
    Пока реализовано то, что нужно. Может еще что-нибудь понадобиться - посмотрим.
...
**** TODO Иногда display включается с gammaBrightness=0
     ...
     Это даосаточно важный момент, поскольку можно оказаться с темным экраном
     и без возможности изменить яркость = перезагрузка.
**** TODO NXDisplay: Resolution, Origin и frame
     Кожется, что изменения должны быть концептульнее. Сейчас frame напрямую
     ...
     frame - сохраненная информация о разрешении, частоте и положении.
     ...
     характеризует монитор является 'frame'. Вот на нее и стот обратить
     пристальное внимание. Заменять Resolution = {} и Origin = {} на Frame
     смысла не имеет. Наоборот, Resolution это копия из списка поддерживаемых
     разрешений, а Origin устанавливается при вызове -setResolution:origin:.
...
**** TODO Конфиг layout'ов
***** TODO Запись конфигов
****** TODO Кто и в каких случаях?
       ...
       2. Изменения извне (xrandr) - Workspace?
       3. Подключение/отключение мониторов - Workspace?
...
******* TODO Запись:
       - (BOOL)[NXScreen writeLayoutAtPath:(NSString*)] или 
         (BOOL)[NXScreen writeLayoutAtPath:(NSString*) forLayout:(NSArray*)]
         - (NSString*)[NXScreen configNameForLayout:(NSArrray*)]
         - (BOOL)[NSArray writeToFile: atomically:YES]
******* TODO Чтение:
       - [NXScreen readLayoutAtPath:(NSString*)]
         - (NSString*)[NXScreen configNameForLayout] - генерация имени файла
         - (NSDictionary*)[NSArray dictioanryWith]
****** TODO Класть конфиги в ~/Library/Peferences/.NextSpace -> Workspace
***** TODO Нужно как-то опеределять layout'ы
      ...
      Таким обрахом, в случае с монитором и телевизором должно быть как
...
**** TODO Управление конфигурациями и взаимодествием видеоадаптеров
     ...
     Поэтому эта задача сводится к правильно устанволенному и настроенному
...
**** Инфо
     ...
     - devd (FreeBSD)
     - hal (FreeBSD, Linux)
*** TODO [?] NXAuth - user authentification, user/groups management
...
***** TODO [#A] Inspector: не вызывать если панель скрыта?
...
*** TODO Модули
**** TODO Нарисовать в Gorm все панели и определиться с размером окна
***** DONE Login
***** TODO Fonts
***** DONE Display
***** DONE Screen
...
***** TODO Устанавливать 'Main Display'
***** TODO Активировать/деактивировать монитор
...
***** TODO Установка Backlight (если монитор поддерживает)
      Нужно вместе с Preferences/Modules/Display. Поэтому, здесь нужно
      сделать паузу для NXSystem/NXScreen.
***** TODO Background color of desktop
...
***** TODO Ввод только цифр и точки в текстовые поля (gamma, brightness)
***** TODO Установка Brightness
***** TODO Color profiles
**** TODO Fonts: GNUstep font settings
**** TODO Localization: language, units, paper, encoding
...
**** TODO Menu, Dock, icons
     ...
     - Dock placement on multi-monitor configuration;
     - WindowMaker Dock settings: Dock position, hiding, etc.
     - Bottom icons placement
**** TODO UNIX Expert
**** TODO Password/Users?
