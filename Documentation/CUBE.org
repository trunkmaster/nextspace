...
**** TODO Проверить работает ли изменение разрешения при увеличении экрана
**** TODO Проверять поддерживает ли дисплей Gamma
**** TODO NXDisplay: заменить Dimensions и Origin на Frame
**** TODO [#A] Background color of desktop
**** TODO [#A] Установка Backlight (если монитор поддерживает)
     Нужно вместе с Preferences/Modules/Display. Поэтому, здесь нужно
     сделать паузу для NXSystem/NXScreen.
**** TODO [#A] События (изменению параметров, подключени/отключение  мониторов)
     ...
     Нужно:
      ...
      - отправить notification для заинтересованных (в первую очередь NXScreen);
     ...
     Как быть?
**** TODO [#B] NXDisplay: rotation & reflection
**** TODO [#C] Управление конфигурациями и взаимодествием видеоадаптеров
**** TODO [#C] Округление значений гаммы до второго знака после запятой
     Не получается. Брошу пока.
...
**** DONE Корректная работа с многомониторными конфигурациями
