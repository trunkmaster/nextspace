...
***** TODO Language
      ...
      Отдельный виджет с перетаскиваемыми языками, но не так, как это сделано
      в NSTableView. NSScrollView с documentView в виде NSMatrix.
***** TODO Formats
      NSUserDefaults:
      - NSTimeFormatString, NSDateFormatString, NSShortDateFormatString
      - NSTimeDateFormatString, NSShortTimeDateFormatString
      - NSCurrencySymbol, NSInternationalCurrencyString, NSCurrencyString
      - NSDecimalSeparator, NSThousandsSeparator
      Валюта: NSCurrencySymbol, NSPositiveCurrencyFormatString
      Числа: 
***** TODO Measurement Unit
...
***** TODO Ввод только цифр и точки в текстовые поля (gamma, brightness)
      Стоит реализовать подклассы в NXAppKit.
      NSTextField->NXIntegerTextField->NXFloatTextField.
      NXIntegerTextField - ввод чисел, без точки, без пробелов.
      NXFloatTextField - если без точки добавлять '.0', если вида '.X'
      добавлять '0' впереди.
