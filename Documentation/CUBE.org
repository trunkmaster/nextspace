...
** Что нужно для релиза
   ...
   - [ ] Возможность настройки важных параметров (шрифты, дисплей, клава, мышь, звук)
...
*** Look and feel
    ...
    - WM+WM: miniwindows (font, placing), pass click, DnD (Workspace<->Dock,
      Alternate button)
...
** TODO Шрифт у иконки нечитаемый
   ...
   у minicon (minwindow). Сейчас у art и WindowMaker разный тип hinting'а -
...
** TODO WPrefs.app
   - Выбрать все существующие настройки
     Нужно поискать по WINGs, WPrefs.app, src. Трудозатратно. Для чего это нужно?
...
**** TODO Применение конфигурации display layout
     Конфигурации изменяются при помощи Preferences.app->Display.
***** TODO Оставлять конфигурацию подключенных мониторов
      ... и применять дефолнтные настройки для новых мониторов.
      Что делать с Layout? 
      На какое место ставить найденный монитор (слева направо, разваливать
      стыковку мониторов и делать дублированное изображение)?      
***** DONE Конфигурация по-умолчанию при запуске Xorg/Login...
      ...если сохраненной конфигурации нет.
      DONE: просто выключает все мониторы, кроме первого в списке если
      мониторов больше 1.
...
***** DONE Workspace.app: размещать Dock и IconYard на mainDisplay
      Сделал в Workspace+WindowMaker.m (XWUpdateScreenInfo()).
**** TODO NXDisplay: определять lid closed/opened
...
**** TODO NXDisplay: заменить Dimensions и Origin на Frame
...
*** TODO [#C] NXPower - power state, battery, hibernate/shutdown/restart/sleep, saver
*** TODO [#C] NXNetwork - hostname, interfaces, routes, user initiated up/down, WiFi
*** TODO [#B] NXDeviceManager
...
**** Инфо
     ...
     - devd (FreeBSD)
     - hal (FreeBSD, Linux)
*** TODO [#C] NXAuth - user authentification, user/groups management
...
** TODO Application implemented Contents Inspectors
...
***** TODO Ловить подключение/отключение мониторов
...
***** TODO Перечитывать список мониторов после изменений
