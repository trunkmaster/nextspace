...
** Текущий видимый план
   1. Applications-Preferences-Keyboard
   2. Applications-Prefernces-Mouse
   3. Terminal: Keyboard and mouse definite correct work
   4. Terminal: activity control, Font menu completeness
   5. NXSystem-NXScreen
   6. Application-Preferences-Display
   7. Workspace and Login correct displays operation
   8. NXAppKit-NXAlertPanel: size changes for multiline messages
   9. Workspace & Preferences: WindowMaker, Reycler, enhancements and bugfixes
   10. NXAppKit-NXSavePanel,NXOpenPanel
   11. NXFoundation-NXBundle
...
** TODO -NXAutoLaunch
*** TODO Должно скрываться не только основное меню, но и остальные
*** TODO Нужно убирать из списка параметров?
    Пока убираю, но остается вопрос как внутри приложения определить вызвано
    ли оно с -NXAutoLaunch.
...
** TODO [NSScreen frame] и изменение рарешения
   Нужно модифицировать back в части XGServerWindow.m. Сейчас widthxheight
   берется с помощью макросов Xllib DisplayWidth, DisplayHeight, которые
   возвращают значения полей структуры Screen (Xlib.h). При изменении
   разрешения эти значения этих полей остаются такими, какими были при старте
   приложения (XOpenDisplay). Возможно нужно будет привлекат XRandR.
** TODO NSBrowser
   Вся моя борьба с NSBrowser в Workspace из-за несовершенства NSBrowser'а.
   - Нелогичное поведение при изменении размера: по моему мнению, если целое
     число колонок с минимальным размером помещается в новый размер - нужно
     увеличивать и отображать количество колонок.
   - setTakesTitleFromPreviousColumn не берет значение предыдущей колонки, но
     и сбрасывает в пустое текущее значение.
   - Cell setShowsFirstResponder:NO показывает рамку. Проблема Cell?
   - Horizontal scroller работает тупо: если кликнуть почти в конце/начале,
     когда должна отобразиться крайняя колонка остается в некрайнем положеннии.
     Должен округлять свое положение в крайнее левое/правое.
** DONE Single click не получает NSIconWindow (appicon)
   Решение находится в gnustep-back/x11/XGServerWindow.m
   processEvent:. Если используется WindowMaker для appicon управление
   передается WindowMaker и выполеняется выход из switch-case без
   установленного 'event'. Поэтому NSEvent не передается приложению и окну.

   DONE: gnustep-back-x11_XGServerEvent.m.patch
...
*** TODO NXSavePanel, NXOpenPanel
    ...
    keyboard focus in browser.
*** TODO NXAlertPanel
    Modified NSRunAlertPanel().
    Нужно проверить еще раз с разным количеством строк. Наблюдал проблемы с
    количеством строк == 3. Третья строка не отображается.
**** DONE Максимальный размер панели
     Если текст messageField не помещается в максимальны размер панели, есть
     варианты:
     1. Обрезать видимый текст. Нужно только убедиться в том, что не видно
        часть строки внизу.
     2. Создавать ScrollView с вертикальным scroll'ом.
     3. Уменьшать размер шрифта, пока текст не поместиться или делать окно
        максимального размера, а текст пускай обрезается.
     Пойду посмотрю в NeXT UIG...
     В UIG сказано о том, что ScrollView может добавляться если требуется
     дополнительные объяснения для сообщения (кнопочка 'Explain' показывает
     этот ScrollView). Поэтому пока останавливаемся на варианте №3.
*** DONE NXIcon
*** DONE NXIconLabel
*** DONE NXIconView
...
**** TODO NXFindPanel
    ...
    В приципе, работает и штатная панель. Может когда-нибудь можно будет
    добавить поиск по regexp. Переношу в 'Version 2.0'.
...
***** DONE Приложения перестали сохранять настройки
...
**** NXDisplay: Resolution, Origin и frame
     ...
     setResolution:origin: приводят к кривому layout. Например:
      ...
      - setResolution:origin: - завершается неуспешно по той же причине, что и
        ...
        Наверное нужнен класс, которы позволит удобно манипулировать содержимым
...
**** DONE Input languages (Xkb layouts)
**** DONE Repeate rate (Xorg)
...
***** TODO [#C] DnD: Shelf, PathView
      - При добавлении иконки в Shelf, нужно явно вызывать [FileViewer
...
***** DONE Не обрабатывать параметры коммандной строки
      Это нужно для того, чтобы можно было указывать параметры GNUstep
      (например, понадобилось запускать Workspace с опцией --GNU-Debug).
      Добавлено в патч src_main.c.patch.
...
******** DONE Привести NSWindow.h и GNUstep.h в WindowMaker к одному виду
         Ну и естетсвенно проверить, что получилось в результате. Если все
         OK:
******** DONE посмотреть код работы с панелями (gnustep-back)
         ...и сделать изменения с учетом новых цифр в NS*WindowLevel.
******* DONE Появление окон Workspace
        ...
        После реализации -NXAutoLaunch стало намного плавнее, без "моргания"
        фокуса на меню и окнах Workspace. Нужно ли дожидаться запуска всех
        приложений в Dock? Думаю нет, поскольку какое-нибудь плохо написанное
        приложение может лишить доступа к desktop.
******* DONE WindowMaker: После перезапуска приложения съзжают вниз
        ...
        (падение, kill -9).
        src_window.c.patch: @@ -1159,6 +1162,15 @@
****** DONE Создать документ с описанием патчей для GNUstep
       Libraries/gnustep/README.md
****** DONE WindowMaker 0.95.7
******* DONE Включить последний стабильный релиз WindoMaker
******* DONE Перегенерировать патчи под новую версию
****** DONE Создать документ с описанием патчей для WindowMaker
       Application/Workspace/WM-PATCHES/README.md
****** DONE Цвет виджетов WINGs
******* DONE Привести в соответствие с GNUstep
        Пока без диалогов WINGs полностью не обойтись, поэтому нужно привести
        цвет в соответствие с GNUstep.
******* DONE Создать и задокументировать патч
...
******* TODO Иконка 0
       ...
       startup.c: StartUp()
         screen.c: wScreenRestoreState()
           dock.c: wDockRestoreState()
                   restore_icon_state()
                   XMapWindow(dpy, aicon->icon->core->window)

       Кажется стало понятно откуда берется "зубчатое" изображение иконки -
       ее рисует WindowMaker из CachedPixmaps (там все иконки какие-то
       странные) перед тем как приложение GNUstep рисует иконку в appicon.
******* TODO Конфигурирование Dock (элементы, поведение)
...
******** TODO Workspace-Preferences-Dock
         Автозапуск приложений, коммандная строка, Icon Image,
         Middle-click launch, DnD command.
******** TODO Dock position
         Normal, Auto raise & lower, Keep on top.
******** TODO Bring here
         По идее это должно быть интегрировано в стандартное меню Windows.
         А с приложениями X11 как?
******** TODO Launch, Hide, Kill
         Без Launch можно и обойтись, а вот Hide и Kill могут понадобиться,
         особенно для приложений X11.
...
******* TODO Перелетание иконок
        Незапущенное приложение может быть идентифицировано парой class.name.
        Для приложений GNUstep все стандартно: <имя>.GNUstep.
        Для приложений X: если есть, во wrapper нужно добавить параметр с
        значением class.name. Если wrappera нет - ничего не делать.
...
******* DONE При старте размещать Dock и Icon Yard на primary display
        Вроде уже сделал. Нужно проверить.
        Side effect: могу залипнуть в Preferences.app/Display. :(
******* DONE Вид miniwindow
******** Preface
...
****** TODO Workspace: некрасивые иконки в Processes
       ...
       Это можно увидет на примере Emacs.
****** TODO Workspace: перезапуск при падениях, kill и т.д.
       Нужно подумать как организовать Restart Workspace без участия
       Login.app.
****** TODO WINGs panels and menus
       Нужно оставить действительно нужные (без которых трудно обойтись).
       - Window Menu (right click on titlebar);
       - Window Attributes Inspector;
       - Dock icon menu;
       - Dock Icon inspector;
       - Attention panels on:
         - inability to autostart app;
         - force kill of window (Ctrl-Click on close titlebar button);
****** TODO WindowMaker RootMenu (Tools->Launcher)
       При правом клике на desktop должно отображаться главное меню
       Workspace.
******* TODO Допилить работу Workspace->Tools->Launcher
        ...и тогда можно обойтись без RootMenu, из которого я запускаю диалог
        "Run" и xterm.
******* TODO Посмотреть какие возможности можно добавить в Workspace
        Например: Windows, Workspaces, Hide Others, Show All, Session.
...
****** TODO internal X error: BadMatch
       ...
       event 23". Что это - ХЗ.
****** TODO Описать в документе все, что касается Look&Feel
       Общая концепция.
       Размеры, вид, шрифты и т.д.
       Умолчания, которые имелись в виду.
       Изменения, которые внесены в GNUstep и WindowMaker.
****** TODO Focus
******* ---- Проблемы
...
******** TODO Панель переключение между приложениями/окнами (Alt-Tab)
...
******** Варианты
         1. OPENSTEP: Фокус получает то окно, на заголовке которого НАЖАЛИ
            кнопку мыши.
         2. Фокус получает то окно, на заголовке которого ОТПУСТИЛИ кнопку мыши.
         3. Фокус окно не получает если его перенесли.

         WindowMaker ведет себя правильно с приложениями X11.

         В случае с приложениями GNUstep - окно перемещается на передний план,
         но title bar отображает фокус (и mainMenu) только после отпускания
         кнопки мыши.
******** Последовательность вызовов
         WindowMaker:
         - event.c: handleButtonPress
         - window.c: titlebarMouseDown
           - actions.c: wSetFocusTo
             - отправка WM_TAKE_FOCUS (получает XGServerEvent.m)
             - window.c: wWindowFocus(NEW) - кроме GNUstep apps
               - window.c: wWindowUnfocus(OLD) - кроме GNUstep apps
           - moveres.c: wMouseMoveWindow

         XGServerEvent.m:
         - processEvent:(XEvent *)
           - _handleTakeFocusAtom:forContext:
             - готовиться NSEvent для постановки в очередь
               type - NSAppKitDefined, subtype - GSAppKitWindowFocusIn
           - event_queue addObject:(NSEvent)
           - NSEvent получает NSWindow (gnustep-gui) в sendEvent:(NSEvent *)

******** TODO Идеи
         По полной правильной реализации идей пока нет.
         Пока можно сделать следующее:
********* TODO window.c
          wWindowFocus(), wWindowUnfocus() - снять условие, что
          заголовки окна не отображают состояние если приложение GNUstep;
********* TODO actions.c
          wSetFocusTo: не передавать атом WM_TAKE_FOCUS если окно уже имеет
          фокус (также добавить проверку не только внутренних пееременных
          WindowMaker, а и через XGetInputFocus);
********* TODO Работа gnustep-back при различных сценариях
          Проверить XGServerEvent и XGServerWindow на предмет корректности
          обработки всех сценариев смены фокуса.
********** смена фокуса внутри одного приложения GNUstep
********** смена фокуса между разными приложениями GNUstep
********** смена фокуса между приложением GNUstep и X11
********** смена фокуса между приложениями X11
******* TODO Клик внутри окна
        Варианты:
        1. Фокус получает то окно, на котором НАЖАЛИ кнопку мыши.
        2. OPENSTEP: Фокус получает то окно, на котором ОТПУСТИЛИ кнопку мыши.
        3. Фокус окно не получает если из неактивного окна что-то перетащили.
        
        GNUstep и WindowMaker ведут себя неправильно - по варианту 1.
        Нужно как в варианте 2.
******* TODO (HACK): Переключение фокуса
        То, что ниже я писал и делал давно. По сути речь идет об
        использовании scr->no_focus_window при переключении между
        Workpaces. Наверное это не очен правильно, поскольку фокуса у
        приложния GNUstep не будет, а меню, скорее всего, будет видно.
        ...
...
******* DONE Старт приложений с опцией '-NXAutoLaunch YES'
        Ожидаемое поведение:
...
***** TODO Recycler
      - иконка в Dock - создавать программно при старте Workspace;
      - если в Dock нет места - располагать как appicon в IconYard;
      - иконку нельзя удалить из Dock (Lock=Yes в WMState);
      - D'n'D в иконку из Workspace;
      - окно с файлами в Recycler (~/.Trash? а как с другими дисками?);
      - File->Empty Recycler;
**** TODO События Xrandr
     Вроде я давно уже все сделал. По крайней мере проверено в т.ч. в
     VirtualBox 4.3.40 - работает (изменение размера окна приводит к
     перемещению Dock и IconYard). Еще раз все проверить?
**** TODO Взаимодействие с Login
***** TODO Log Out
***** TODO Power Off
      Workspace отправляет Login сообщения о PowerOff если выбран режим выхода
      'Power Off'.
...
*** Версия > 1.0    [0/10]
    ...
    :COOKIE_DATA: todo reccursive
    :END:
**** TODO Перейти на последний стабильный релиз WindowMaker
**** TODO CInspector
...
**** TODO Processes->Background: имена в полях From: и To:
     При манипуляциях с объектами с длинными путями текст в полях From: и To:
     не всегда отличается. Нужно подумать на предмет реализации сокращения с
     точками внутри.
     Задачка не совсем простая, поскольку нужно решить какие части начала и
     конца пути стоит оставлять (и самое главное, как это определить
     программно!).
...
*** DONE Старт без без появления меню
    В Workspace и gnuste-back реализована опция -NXAutoLaunch, которая
    приводит к желаемому результату. Теперь так стартуют еще Weather и
    TimeMon.
*** TODO Применение настроек при старте сессии
    Нужно подумать как и где применять настройки при старте сессии.
    Это касается:
    - инициализации дисплееев 		-> Workspace
    + настройки клавиатуры 		-> Preferences (via NXSystem)
    - настроек мыши и тачпада		-> Preferences (via NXSystem)
    - громкость звука			-> Preferences (via NXSystem)
    - активизации сетевых настроек 	-> NetworkManager?
*** TODO Модули
...
**** DONE Keyboard
***** DONE Layouts
****** DONE Honour 'Modifiers section'
***** DONE Repeat Rate
***** DONE Numeric Keypad
****** DONE Delete Key
****** DONE Behaviour
****** DONE Num Lock state upon startup (login)
***** DONE Modifiers
****** DONE Command/Alternate Swap
****** DONE Compose Character Key
****** DONE Caps Lock Key (Alpha Lock)
**** TODO Mouse
***** DONE Acceleration
***** TODO Double-Click Delay
****** TODO Write WindowMaker and NSGlobalDomain
****** TODO Implement using option in GNUstep backend
***** TODO Mouse wheel scroll
****** TODO Write to NSGlobalDomain
****** TODO Implement using option in NSScrollView
       Option is a multiplier to the scroll height value.
***** TODO Pointers
...
***** TODO Получать и изменять desktop background -> NXDisplay
      ...
      сохранен в ~/Library/Preferences/.NextSpace/NXGlobalDomain.
...
**** TODO UNIX Expert
     - [ ] File Creation Mask
     - [ ] Show hidden files
     - [ ] Должно использоваться Workspace Manager'ом, NSSavePanel, NSOpenPanel.
***** TODO Нарисовать панель
**** TODO Network
***** TODO Нарисовать панель
...
**** TODO Shortcuts
     GNUstep and WindowMaker (learn xmodifier.c).
     Я прихожу к мнению, что Shortcuts больше относиться к WindowMaker, чем
     к Preferences. А значит эти настройки должны пойти в Workspace.
...
*** TODO .preferences as document associated with Preferences
...
**** DONE Preferences
***** DONE Shell
****** DONE Не добавлять дублирующиеся значения в PopUp
       Например, если есть "/bin/sh", то "/usr/bin/sh" уже не добавляется.
***** DONE Startup
****** DONE Startup File
       Implement setting startup file with option "Open the startup file" in
       "When Terminal Starts Up" section.
****** DONE When Terminal Starts Up
       - "Do nothing" - opens Terminal and creates no window.
       - "Open the startup file" - creates Terminal window with startup file
         that set in "Startup File" block.
       - "Create a new shell window" - creates Terminal window with default
         settings.
****** DONE Auto-Launch
       Сейчас уже легко это можно реализовать с опцией -NXAutoLaunch.
       Нужно только не показывать или прятать окно терминала для режимов "Open
       the startup file" и "Create a new shell window".
***** DONE Sessions
****** Поддержка сессий это:
       - возможность сохранять текущие настройки в отдельном файле 
         (меню Shell->Save As...) в файлы ~/Library/Terminal/*.term;
       - возможность выбрать из меню Shell->Open... сохраненные
         настройки в файлах ~/Library/Terminal;
       - меню "Shell->Save":
         - выбрасывает диалог, если сессия не загружена;
         - сохраняет настройки в файл загруженной сессии.
       - в настройке Preferences->StartUp->StartUp File указывается загружаемая
         сессия по-умолчанию;
       - меню "Shell->New" загружает то, что указано в
         Preferences->StartUp->StartUp File или настройки по-умолчанию

       Terminal.plist:
       - загружается по-молчанию если не указан параметр "Startup File" в
         Preferences;
       - меняется при нажатии на кнопку "Set Default" в Preferences;

       <StartUpFile>.term:
       - создается только через "Shell->Save" | "Shell->Save As";
       - меняется только через "Shell->Save".
****** Мысли, наброски
       _Ициализация:_
       1. *Shell | New*
       [Controller openWidndow]
         [Controller newWindowWithShell]
           [Controller createTerminalWindow]
             [TerminalWindowController init]  <--- создание экземпляра Defaults (владелец)
               defaults = [Defaults init]
       2. *Shell | Open...* (еще не сделано, но так должно быть.)
       [Controller openWidndow]
         [Controller newWindowWithShell]
           [Controller createTerminalWindow]
             [TerminalWindowController initWithStartupFile:]
               defaults = [Defaults initWithFile:] <--- владелец TerminalWindowController

       3. Дальше, созданный экземляр Defaults распространяется так:
          [TerminalView initWithPreferences:defaults]
            [TerminalParser_Linux initWithTerminalScreen:width:height:]
          TerminalScreen == TerminalView, дает доступ к Defaults через метод
          -preferences
       
       _Изменение_
       1. Все классы получают доступ к экземпляру Defaults.
       2. Изменения вносятся с помощью сеттеров.
       3. synchronize в Defaults по-умолчанию не должен вызывваться -
          вызывается классом, который вносит изменения.

       _Сохранение изменений_
       1. Info | Preferences | "Set Default"
         [Defaults synchronize]
           [NSUserDefaults synchronize]
       2. Shell | Save, Shell | Save As...
         [Defaults synchronize]
           [NSMutableDictionary writeToFile:atomically:]

       _Доступ к настройкам:_
       - Из Info | Preferences:
         [Preferences mainWindowPreferences]
           [(Controller *)[NSApp delegate] preferencesForWindow]
             [TerminalWindowController preferences] - обращение к владельцу Defaults
       - Из TerminalView и TerminalParser:
         - при инициализации TerminalWindowController передает ссылку на
           экземляр Defaults. 
           [TerminalView initWithPreferences:defaults]
             [TerminalView initWithFrame:]
               [TerminalParser initWithTerminalScreen:(TerminalView)]
                 [(TerminalScreen)TerminalView preferences]
         - дальше они обращаются к экземляру Defaults (defaults), получая
           ссылку у TerminalWindowController.
****** DONE Save As
******* DONE Сохранять множество окон и вписывать в StartupFile
******* DONE Переработать создание окон из session файла
******* DONE Save для загруженного файла с множеством окон
        Как должен рабтать в этом случае Shell->Save? Варианты:
        1. Сохраняется только main window но через Save As.
        2. Сохраняются все открытые  в данный момент окна.
        3. Сохраняются все окна, которые были загружены из файла.
        Можно сделать так:
        - читаем содержимое файла, указанного в representedFilename;
        - если файла нет, запускаем Save As;
        - если файл есть читаем значение MultipleWindows: если YES -
          сохраняем все окна, у которых representedFilename такой же.
******* DONE Проверить работу нового кода
****** DONE Адаптировать Shell под Sessions
       Нужно сделать кнопки, чтобы была возможность в Session сохранять и
       команду, которая запускается в терминале.
       Добавить кнопки "Set Default", "Show Default" и "Set Window".
       Варианты:
       1. Одно поле с названием Shell, в которое можно указывать любую
          команду.
       2. Сделать 2 поля Shell с выбором из выпадающего списка (содержимое
          файла /etc/shells) и поля Command, в которое вписывается команда.
****** DONE Создать сеттеры для всех настроек в Defaults
****** DONE Defaults: добавить метод сохранения в файл
****** DONE Shell | Save
       Сохранет сессию, если была открыта в файл .term.
       Выбрасывает диалог 'Save As' если были загружены настройки
       по-умолчанию.
****** DONE Shell | Save As
******* TODO Реализовать возможность сохраненния нескольких окон.
        Accesory view -> "What to save: Main Window | All Windows".
******* DONE Диалог 'Save As'.
        Если были загружены настройки по-умолчанию предлагаемое имя:
        Default.term; каталог стандарный для хранения сессий
        (~/Library/Terminal).
        Если была загружена сессия, предлагать имя <имя сессии>.term.
****** DONE Shell | Open
       Открывает диалог 'Open' с каталогом ~/Library/Terminal.
***** DONE Colors
****** DONE Cursor
****** DONE Window
****** DONE Text
****** DONE Черный шрифт на белом фоне by default
***** DONE Window и Title Bar
****** DONE при обновлении размера окна, поля Rows и Columns должны обновляться.
       Получилось так:
       - при изменении размера окно шлет TerminalView -setFrame: или
         -setFrameSize:;
       - TerminalView шлет Notification об изменении размера окна;
       - Notification ловит TerminalWindow (-updateWindowSize:), меняет
         livePreferences, шлет notification об изменении размера
         TerminalWindow;
       - Preferences/WindowPrefs ловит последний notification и обнвляет поля.
***** DONE Fonts
****** DONE Font menu
       Устанавливать шриф для main window.
       Соответственно модифицировать значения в Preferences (Window и Colors).
       Нужно реализовать validateMenuItem в правильных местах. Сейчас это
       TerminalView для 'Clear Buffer'.
******* DONE Copy Font, Paste Font
        Я не понял есть ли какое-то соглашение на этот счет. Судя по
        исходникам TextEdit и документации Apple такого соглашения нет.
        Но есть реализация copyFont: и pasteFont: в NSTextView. Я думаю, что
        стоит ориетироваться на эту реализацию (а именн: писать в NSPasteboard
        информациюо шрифте как в NSTextView). В таком случае, будет
        совместимость с любым наследником NSTextView.
        Реализация будет считаться правильной если можно будет сделать Paste
        Font из сделанного Copy Font TextEdit.
        TextEdit при "Copy Font" создает NSDictionary:
        {
          @"NSColor" = *NSColor;
          @"NSFont" = *NSFont;
        }
******* DONE Bold
        В OPENSTEP пукты 'Bold' и 'Italic' не активны всегда (или я не не
        нашел, в каком случае они активируются).
        Нужно ли менять на Bold Off и обратно? Не нужно, оставляю как есть.
******* DONE Italic
        В OPENSTEP пукты 'Bold' и 'Italic' не активны всегда (или я не не
        нашел, в каком случае они активируются).
        Нужно ли менять на Italic Off? Не нужно, оставляю как есть.
******* DONE Font Panel
        Устанавлиавет шрифт активного окна. Если нет окон - пункт меню
        недоступен.
******* DONE Larger
******* DONE Smaller
****** DONE Window Preferences
       При изменении шрифта, пересчитывать и устанавливать заново resize
       increment.
       UPDATE: Сделалн, но все немного серьезней - нужно сделать review коду,
       который устанавливает размер TerminalView. Создал новую задачу "Code
       review..."
****** DONE Обновлять FontPanel 
       ...устанавливать шрифт выбранного окна в FontPanel открытого из
       Preferences если окно терминала открыто.
***** DONE При изменении main window считывать настройки с него
      Похоже нужно пересмотреть подход к хранению и обновлению настроек.
      Сейчас кажется логичным следующее:
      - при вызове панели Preferences нужно, чтобы отображались настройки
        main window;
      - отображение настроек в UserDefaults должно показываться только по
        нажатию кнопки "Show Default";
      - при изменении настройки окна все, кто читает настройки этого окна,
        должны перечитывать; например:
        - открыто окно "Preferences-Title Bar", "Custom Title" == Off;
        - открывает окно "Set Title" и меняем title, нажимаем кнопку "Set";
        - Preferences делают "Custom Title" == On, меняют значение TextField
          и обновляют текст sample;
        - заголовок окна меняется с учетом установок Preferences и нового
          Title.

      В итоге, обрабатываться должно 2 типа настроек:
      1. То, что храниться в UerDefaults (класс Defaults).
      2. То, чем оперирует окно (класс TerminalWindow). Эти же настройки
         хранятся в файле сессии *.term. Сохраняются при вызове пункта меню
         "Shell->Save" и "Shell->Save As...".
      При вызове "Preferences", "Set Title" настройки запрашиваются у
      TerminalWindow.
      При нажатии на кнопку "Set Default" отображаются настройки загруженные
      из файла (NSUserDefaults или .term).
***** DONE При изменении шрифта пересчитывать Bold
...
**** DONE Services
***** DONE Панель Info->Terminal Services
****** DONE При создании .plist проверять Send/Return Types
****** DONE Customized service name edit in list
****** DONE Use customized edit field for New service
****** DONE Command line tooltip
****** DONE outputMatrix
       В зависимости от выбора в executeTypeBtn менять надписи:
       ...
       + New Window
       - Idle Window
****** DONE "Save" should be disabled if "OK" enabled
****** DONE shellMatrix disabled если выбрана опция "Idle Window"
****** DONE Implement "Save..." button
       "Save Service" panel with aux view with services selection to save to
       alternate location (plist file).
****** DONE Статус кнопки "OK"
       After "OK" button pressed futher changes won't change state of "OK"
       button and panel.
****** DONE Первый запуск панели Terminal Services
       Если определенных сервисов нет, спросить у пользователя загружать ли
       сэмплы (посмотреть в OPENSTEP).
****** DONE При добавлении проверять дубли
****** DONE Добавление сервиса и фокус
       При добавлении сервиса после ввода его имени переводить фокус на поле
       "Command and Key Equivalent".
       По большому счету Command Field должно быть всегда в фокусе.
***** DONE Implement "Add Arguments" panel
***** DONE Проверить запуск сервисов с различными параметрами
****** DONE NSSendTypes и %s
       При генерации TerminalServices.plist NSSendTypes - для "On Cmd Line" -
       нужно добавлять только в том случае если в коммандной строке
       присутствует %s.
       Cases:
       1. NSSendTypes есть (%s, On Cmd Line, Plain Text):
          - текст не выделен, пункт меню неактивен.
          - текс выделен, пукт меню активен, %s заменяется, без %s добавляется.
       2. NSSendTypes нет (On Cmd Line, Plain Text)
       Согласно документации Apple, NSSendTypes опциональное поле. Однако если
       у сервиса нет такого поля, ему не передается Selection. Это вполне
       логично, поскольку GNUstep не знает какую информацию можно передавать
       сервису. Поэтому сервис не может принимать selection и при этом быть
       доступным при отсутствии selection.
       Для того, чтобы создать сервис, который принимает переменное число
       аргументов нужно создать сервис с параметром %p. NSSendTypes не
       создаются, соответственно selection не передается. Но при вызове
       такого сервиса появляется панель "Add Arguments" в которую можно
       сделать Paste или удалить выделенный %p для запуска команды без
       параметра.
****** DONE %s
****** DONE %p
****** DONE %s %p
***** DONE Проверить типы создания окон (shell, program)
****** Do we need "Idle" window type?
       The answer is: not explicitly. Use it internally for running programs
       and shells.
// Window type and state:
// 1. Normal - has running shell/command inside it. No other command was
//             executed inside shell. Close button has normal state.
//             Miniwindow displays animation on produced output.
// 2. Edited - User start command/program inside shell.
//             Programm running that started through Services.
// 3. Idle   - 1) Command that started through "Services" exited.
//                "[Process exited]" message printed.
//                Close button has normal state.
//             2) Shell/command exited and "When shell Exits" set to
//                "Close the window if shell exitst cleanly" or
//                "Never close the window".
//                "[Process exited]" message was printed.
//                Close button has normal state.

// Commands running through Services menu in window must ignore
// "When Shell Exits" preferences settings. Such windows must behave in this
// way:
// - while command is running window close button display edited state.
// - when command exited window close button displays normal state and
//  "[Process exited]" message displayed in window.
// If window created through the -newWindowWithProgram:arguments:input: method
// some events occured during window lifecycle:
// - new window has added to 'windows' dictionary;
// - when shell/command exited -terminalWindow:becameIdle: method has called;
// - when -checkTerminalWindowsState called window close button may be changed.
// For Services command window we should ignore first two events.
****** DONE Вернуть методы работы с Idle Window
       В свете polymorphicOpts нужно пересмотреть вопрос о Idle.
       Idle Window это просто первое окно, в котором ничего не
       выполняется. В это окно:
       - передается выполнение команды;
       или
       - заменяется shell на выполняемую комманду.
****** DONE Run in shell only if ExecuteInShell == 1
**** DONE Edit->Select All
***** DONE Выбирать не только видимый текст.
***** DONE Корректный скролл
      Несколько первых строчек некорректно отрисовываются при скролле.
**** DONE Mouse selection & last char at EOL
**** DONE Edit->Find
     Искать по содержимому буфера. Если найдено скроллить к найденому.
**** TODO (After: Preferences.app-Keyboard) Keyboard 
     Super (Win)
     - Terminal		Meta (Terminal Preferences > Linux Emulation >
       			Alternate Key > Alternate key generates Escape
       			sequences).
     - WindowMaker	Mod4 (используется почти во всех shortcut'ах)
     - GNUstep		Alternate (по идее нигде не используется)
***** TODO Keyboard
      Command, Alternate, Control, Shift.
      Проверить как передаются сочетания клавиш с этими модификаторами.
      Сейчас известно, что Command и Shift не передаются в любом случае.

      _Control_

      Control-Home, Control-End не работает в Emacs. Похоже это такая же
      ситуация как и с Shift.

      _Alternate и Command_

      Если при нажатом Alt(Command) или Super(Alternate) передавать '\e':
      - <Super+кнопка> работает всегда.
      - <Alt+кнопка> - работает, если нет такого сочетания в меню. Другими
        словами, сочетания клавиш app menu выше приоритетом.

      Design rules:
      1. Command используется исключительно для shortcut'ов приложений
         GNUstep.
      2. Alternate оправдывает свое название и может быть использована там,
         где предполагается использоваение Alt. По-умолчанию в NextSpace
         Alternate назначен на кнопку 'Super', Command на "Alt".
      Таким образом, кнопка Command(Alt) всегда используется для привычных
      действий в других приложениях (Cmd-C - Copy, Cmd-V - Paste и
      т.д.). Alternate используется в:
      ...
      Нужно это все где-то описать, чтобы пользователям было понятно.
      
      _Shift и другие сочетания модификаторов (Shift-Alt, Ctrl-Alt-Shift)_

      Терминальные программы пользуются ncurses, которая в свою очередь
      пользуется terminfo. Terminal изначально был реализован с
      использованием парсера Linux Console. Соответственно при запуске
      приложений переменная TERM выставляется в значение 'linux'.
      Файл terminfo 'linux' описывает возможности консоли Linux. Консоль
      Linux не поддерживает модификатор Shift.

      В xterm эта проблема решена следующим образом в файл terminfo внесены
      расширения, которые описывают char sequences для (например, для
      Shift-Up это '\e1;2A'. Соответственно, приложение терминала может
      транслировать нажатие сочетания клавиш в соответствующий char sequence.

      По-идее всю недостающую функциональность нужно добавить в terminfo-файл
      linux, назвать его linuxterm, положить его в ~/.terminfo и выставлять
      переменную TERM=linuxterm при создании окна терминала.
      Но видимо не все так просто. Просто перекомпилированный файл xterm
      работает не так (например, Shift-Right/Left работает, а Shift-Up/Down -
      нет).

***** TODO Preferences | Linux Emulation
      Проверить как работают и сохраняются настройки из раздела "Linux
      Emulation".
***** TODO Отслеживать нажатия Crtl+s и Crtl+q
      И добавлять в title bar "IO Stopped" (для Crtl+s).
**** TODO (After: Preferences.app-Mouse) Скролл колесом мышки
     Нужно скроллить как указано в Preferences.app.
     Настройки мыши, которые устанавливаются в Preferences должны читаться
     Workspace(WindowMaker) и другими приложениями(в данном случае Terminal).
     NXMouseWheelScroll - как скроллить обычном режиме.
     Control + Scroll - 1 line.
     Shift + Scroll - 1 page.
**** TODO Activity control
***** TODO Раздел Preferences "Activity Monitor"
***** TODO Window state
      Если как shell указано что-то из /etc/shells, новое окно не дложно
      показывать "Edited" статус.
      Если запускается любая другая команда (например, 'ssh') - всегда
      показывать "Edited" - нет возможности ловить запуск команды в стартовой
      оболочке. Например, для 'ssh' команды запускаются на удаленном сервере...
      Для команд запущенных через Services "Edited" показывается пока команда
      выполняется и "Normal" когда команда завершилась
***** TODO Процессы в окне терминала
      Нужно как-то отслеживать запускаемые процессы в окне терминала.
      Сейчас это реализовано через tcgetpgrp (ID группы процессов активного
      терминала).
***** TODO "Clean" Commands
      Реализовать обнаружение команд, выполнение которых не должно менят
      статус окна на "Edited".
      Начало положено: в Controller реализован метод isProgramClean
***** TODO Miniwindow
      Пока на уровне идеи:
      - добавлять в иконку приложения иконку со статусом приложения (killed,
        stopped, suspended);
      - отображать scrolling "Out" image если, что-то меняется.
**** TODO Font->Colors
     Перетаскивание цвета на фон окна, текст, курсор в окне должно менять эти
     цвета.
**** TODO Info Panel
     С прокручивающимся текстом в иконке терминала. ;)
**** TODO Code review for set window and view size
...
**** DONE Расположение на экране новосозданных окон
     ...
     мне не нравится.
**** DONE Шрифт Roboto рисуется с большими интервалами
     Возможно это из-за того, что шфрифт декларируется как немоноширинный.
     Нет, все было из-за того, что для определения ширины шрифта
     использовался метод boundingBoxForFont. Заменил на advancementForGlyph и
     все стало хорошо.
**** DONE Отображение нормального текста
     Должно отображаться след. образом:
     - если FG и BG совпадают - из настроек;
     - если цвет BG черный и blackOnWhite - не инвертировать;
     - если цвет BG не черный - заданным цветом;
**** DONE White on Black
     ...
     Нужно подумать на предмет определения выставленного цвета по-умолчанию.
     Тогда можно будет отличить выставленный White On Black от цветов
     по-умолчанию.
**** DONE Reverse video
     Если прокручивать 'man man' после строки 9 (отобржается пустая строка),
     у нижней статусной строка (начинается с " Manual page" отображается в
     инвертированном режиме) цвет текста меняется на цвет фона окна.
     Это не видно при белом цвете фона окна, поскольку цвет совпадает с INV_FG.
**** DONE Выделять blink цветом, определенным в Preferences
**** DONE Программа top не рисует заголовок (названия ресурсов процессов)
     Скорее всего просто рисуется белым по белому.
**** DONE Выделять bold цветом и шрифтом
**** DONE Set Title
     В целом оперирует настройкой "Custom Title".
     Устанавливает параметр "Custom Title" в значение формата: 
     	<CustomTitle>|Terminal -- <настройки Preferences->Title Bar> -- <FileName>
     - через меню Shell->Set Title:
       - устанавливает галочку "Custom Title" в Preferences->Title Bar;
       - меняет значение текстового поля "Custom Title";
       - обновляет заголовок текущего окна согласно установленному;
     - если делать все тоже самое вручную через Preferences->Title Bar
       заголовок активного окна меняется только после того, как будет нажата
       кнопка "Set Window".
*** TODO > 1.0
**** TODO Preferences
***** TODO Параметры для Shell
      Варианты:
      1. При выборе какого-то shell из PopUp метка "Command:" меняется на
         "Shell Parameters:".
      2. Выбор из выпадающего списка приводит к вставке в поле
         "Command". Далее пользователь добавляет параметры, которые нужно.
      С другой стороны пользотелю ничего не мешает выбрать "Command" из
      выпадающего списка и вписать туда то, что ему хочется. Пока отбой и
      перемещаю в версию > 1.0 на подумать.
***** TODO Display -> Scrollback Buffer
      Сейчас значение Unlimited приводит к установке значения lines=1024.
      Нужно реализовать lazy механизм выделения памяти.
***** TODO Display -> Wrap lines that are too long
***** TODO Colors -> Cursor -> Blinking
...
** 8. Weather
   Индикатор погоды: температура, влажность
*** DONE Сделать control с вдавленным или выпуклым текстом
    Может пригодится не только здесь, но и в Workspace для отображения
    текущего workspace.
    SunkenString в WeatherView.
*** DONE Отображение температуры и влажности на appicon
*** TODO Пункт меню: "Refresh"
**** DONE Сделать работающим
**** TODO Отображать на appicon статус обновления
**** TODO Сделать что-то с тайм-аутом при остуствии соединения
*** TODO Настройки
**** TODO Город
**** TODO Частота обновления
**** TODO Units: celsius, farenheit
*** TODO Панель с прогнозом погоды (Forecast)
**** TODO Несколько городов на панели
...
** Результаты экспериментов
   ~/fonts.conf - вляет на приложениями, которые используют fontconfig.
   На данный момент у меня это: WindoMaker, приложения GTK и Qt.
   В gnustep-back ART не используется fontconfig, какие шрифты использовать
   (bitmap или outline) решает сам backend.

   Helvetica-Medium.ttf - содержит bitmap шрифты для размеров 8,10,12,14.
   Обнаружил: для того, чтобы визуально шрифты были похожи на OPENSTEP нужно
   использовать:
   - размер на 1 pixelsize больше чем в OPENSTEP, кроме размера 12 (шрифт
     перенесен из .pcf);
   - ширина строки Helvetica шире чем такая же Arial (например, из-за более
     широкой русской буквы 'к', другого кернинга и т.д.);
   - можно говорить, что Arial и Helvetica метрически идентичные поскольку
     метрики совпадают.


** Antializing
   - для приложений fontconfig: использовать bitmap для шрифтов с pixelsize 12
     и меньше;
   - в GNUstep back-art жестко вшито использовть bitmap для определенные
     размеров;
   - в NSFont есть возможность выбирать тип отрисовки шрифтов
     NSFontRenderingMode; может быть использован с помощью метода
     screenFontWithRenderingMode:(NSFontRenderingMode)mode;
   - идеальным вариантом было бы:
     - использование режима NSFontAntialiasedRenderingMode если установлена
       настройки отображать шрифты со сглаживанием;
     - для размеров указанных в настройках использовать bitmap;
     - приложения, которые имеют свои настройки antialising сами создают
       NSFont и устанавливают NSFontRenderingMode.

*** TODO Проверить ручное управление antialiasing с помощью NSFont
*** TODO Где зашиты размеры bitmap в back-art?
*** TODO Какой NSFontRenderingMode используется в GUI по-умлчанию?


** Создание шрифта
   Делается так: 
   1. Взять BDF или PCF из пакета для X11 с кодировкой iso10646 и разрешением
      75x75.
   2. Берем TTF и создаем в нем Bitmap Strikes нужных размеров с включенной
      галочкой "Create Rasterized Strikes (Not empy ones)".
   3. Делаем импорт BDF/PCF в TTF. На вопрос fontforge заменять ли
      сущетсвующие Bitmap Strikes отвечаем: "да".
   4. Символы, которые есть в outline, но нет в bitmap, fontforge отмечает
      красной рамкой. Для этих символов можно сгенерировать bitmap (Ctrl+B или
      меню "Element->Regenrate Bitmap Glyphs...")
   5. Правим начертания некрасивых bitmap'ов, вставляем недостающие.

   В итоге получается шрифт, который:
   - имеет outline для тех программ, которые хотят рисовать сглаженные шрифты
     во всех размерах;
   - для умных есть bitmap шрифты для некоторго набора размеров;
   - bitmap шрифты из набора шрифтов X11 + сгенерированные из outline -
     получается набор глифов, который соответствует outline.


** TODO Helvetica
   Outline: Helvetica World (основной), Arial (источник дополнительных
   глифов)
   Bitmap: AdobeMy - шрифты X11 с добавленными кириллическими символами
*** TODO Helvetica Medium
**** TODO Добавить символы из Arial и сгенерить Bitmap Strikes
*** TODO Helvetica Bold
*** TODO Helvetica Oblique
*** TODO Helvetica Bold Oblique


** TODO Ohlfs
   Можно взять за основу Liberation Mono и добавить ручные Bitmap Strikes.
   Битмапы можно брать из моего вручную сделанного ширфта и из Terminus.
