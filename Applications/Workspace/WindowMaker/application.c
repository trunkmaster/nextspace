...
wApplicationCreate(WWindow *wwin)
{
  ...
  // STOIAN: Tell Workspace to track application launch
  dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH,0), 
		^{ xApplicationDidCreate(wapp); });
...
wApplicationDestroy(WApplication *wapp)
{
    ...
    //-->STOIAN: Tell Workspace to track application closing
    dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH,0), 
  		  ^{ xApplicationDidDestroy(wapp); });
