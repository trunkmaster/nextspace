  ...
  while ( (eachFileType = [overFileTypes nextObject]) )
      ...
      if ( [extensions isKindOfClass:[NSArray class]])
	{
	  ...
	  for (i=0; i < [extensions count]; i++)
	    {
	      [theArray addObject:[[extensions objectAtIndex:i] 
...
//  NSLog(@"fileConfig: %@", fileConfig);
  ...
  if (!unarchiveDirectoryPath)
    {
      ...
      result = NSRunAlertPanel(@"OpenUp",
			       NSLocalizedString(@"DecompressionFailedTempDirectoryFailed",@"Unable to create temp directory in %@"),
  ...
  substitutionKeysForWrappedPrograms = 
    [self wrappedProgramsUsingConfiguration:fileConfig];
  ...
  replaceValues = [NSMutableArray arrayWithObjects:
    [archivePath stringWithShellCharactersQuoted],
