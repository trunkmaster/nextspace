...
- (struct card *)_enumerateCards
{
  ...
  first_card.indexstr = "-";
  first_card.name = "Default Sound Card";
...
- (void)_openMixerWithName:(char *)device_name
{
...
- (void)awakeFromNib
{
  ...
  elementsView = [[NSScrollView alloc] initWithFrame:NSMakeRect(-2,-2,402,536)];
  [elementsView setHasVerticalScroller:YES];
  [elementsView setHasHorizontalScroller:NO];
  [elementsView setBorderType:NSBezelBorder];
  [[alsaWindow contentView] addSubview:elementsView];

  [elementsView addSubview:[[[ALSAElement alloc] init] view]];
  ...
  // elementsMatrix = [[NSMatrix alloc] initWithFrame:[elementsView documentVisibleRect]
  //                                             mode:NSListModeMatrix
  //                                        cellClass:[ALSAElement class]
  //                                     numberOfRows:1
  //                                  numberOfColumns:1];
} 
