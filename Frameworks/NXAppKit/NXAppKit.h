/*
  NXAppKit.h

  The main include file of the NXAppKit framework.

  This file includes all required header files in order to use
  all features in the NXAppKit framework.
*/

#import <NXAppKit/NXIcon.h>
#import <NXAppKit/NXIconLabel.h>
#import <NXAppKit/NXIconView.h>
#import <NXAppKit/NXIconView.h>
#import <NXAppKit/NXProgressBar.h>
#import <NXAppKit/NXProgressPie.h>
#import <NXAppKit/NXProgressView.h>
#import <NXAppKit/NXSplitView.h>
#import <NXAppKit/NXSizer.h>
#import <NXAppKit/NXClockView.h>
#import <NXAppKit/NXUtilities.h>

// Moved into Workspace.app
//#import <NXAppKit/NXPathView.h>
//#import <NXAppKit/NXShelfView.h>
//#import <NXAppKit/NXShelfIcon.h>
