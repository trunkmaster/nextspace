...
void fadeInFadeOutTest(OSEScreen *sScreen)
{
...
void gammaCorrectionTest(OSEScreen *sScreen)
{
  // NSArray   *displays = [sScreen connectedDisplays];
  // for (OSEDisplay *display in displays)
    // {
...
void screenInfo(OSEScreen *screen)
{
  NSSize size;
  ...
  for (OSEDisplay *d in displays)
    {
      ...
      fprintf(stderr, "%s\t  %s\t\t%s\t  %s\t%s\t%.0f\t%4.0fx%4.0f\t%.0f,%.0f\n",
              ...
              ([d isBuiltin] && [OSEPower isLidClosed]) ? "Closed" : "Opened",
              ([d isActive]) ? "Yes" : "No",
              ([d isMain]) ? "Yes" : "No",
              [d dpi],
              size.width, size.height,
...
void displayDetails(OSEDisplay *display)
{
  NSSize  size;
  NSPoint point;
  ...
  size = NSSizeFromString([[display bestResolution] objectForKey:OSEDisplaySizeKey]);
