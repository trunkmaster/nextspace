...
static OSEDefaults *sharedSystemDefaults;
static OSEDefaults *sharedUserDefaults;
static OSEDefaults *sharedGlobalUserDefaults;
static NSLock     *syncLock;

@implementation OSEDefaults
...
+ (OSEDefaults *)systemDefaults
{
  if (sharedSystemDefaults == nil) {
    sharedSystemDefaults = [[OSEDefaults alloc] initWithSystemDefaults];
  }
  return sharedSystemDefaults;
}

+ (OSEDefaults *)userDefaults
{
  if (sharedUserDefaults == nil) {
    sharedUserDefaults = [[OSEDefaults alloc] initWithUserDefaults];
  }
  return sharedUserDefaults;
}

+ (OSEDefaults *)globalUserDefaults
{
  if (sharedGlobalUserDefaults == nil) {
    sharedGlobalUserDefaults = [[OSEDefaults alloc] initWithGlobalUserDefaults];
  }
...
- (OSEDefaults *)initDefaultsWithPath:(NSSearchPathDomainMask)domainMask
...
- (OSEDefaults *)initWithSystemDefaults
{
  isGlobal = NO;
  isSystem = YES;
...
- (OSEDefaults *)initWithUserDefaults
{
  isGlobal = NO;
  isSystem = NO;
...
- (OSEDefaults *)initWithGlobalUserDefaults
{
  isGlobal = YES;
  isSystem = NO;
  return [self initDefaultsWithPath:NSUserDomainMask
  ...
  if (syncTimer == nil || ![syncTimer isValid]) {
    syncTimer = [NSTimer scheduledTimerWithTimeInterval:2.0
                                               ...
                                               userInfo:nil
                                                repeats:NO];
    [syncTimer retain];
    NSDebugLLog(@"OSEDefaults", @"[OSEDefaults] Timer scheduled!");
  }
}
...
- (BOOL)synchronize
{
  ...
  NSDebugLLog(@"OSEDefaults",
              @"[OSEDefaults] Waiting for defaults to be synchronized. %@ - %i",
              syncTimer, [syncTimer isValid]);
  if (syncTimer != nil && ([syncTimer isValid] != NO)) {
    [syncTimer fire];
  }
  NSDebugLLog(@"OSEDefaults", @"[OSEDefaults] synchronized!");
...
- (void)writeToDisk
{
  ...
  NSDebugLLog(@"OSEDefaults", @"[OSEDefaults] write to file: %@", filePath);
