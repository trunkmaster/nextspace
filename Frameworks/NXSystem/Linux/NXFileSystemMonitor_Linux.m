...
@implementation NXFileSystemMonitorThread (Linux)
...
- (id)initWithConnection:(NSConnection *)conn
{
  ...
  if (in_fd < 0)
	...
	{
	  NSLog(@"NXFileSystemMonitorThread(Linux): Could not open inotify(7)"
                " descriptor. Error: %s.\n", strerror(errno));
	}
    }

  monitorOwner = (NXFileSystemMonitor *)[conn rootProxy];
...
- (void)dealloc
{
  NSDebugLLog(@"NXFileSystemMonitor", @"NXFileSystemMonitorThread(Linux): dealloc");
...
- (void)_addPath:(NSString *)absolutePath
{
  ...
  if (in_fd < 0)
    {
      NSLog(@"NXFileSystemMonitorThread(Linux): ERROR trying to add path "
  ...
  if (absolutePath == nil || [absolutePath isEqualToString:@""])
    {
      NSLog(@"NXFileSystemMonitorThread(Linux): ERROR trying to add empty path!");
      return;
    }
  ...
  NSDebugLLog(@"NXFileSystemMonitor",
              @"NXFileSystemMonitorThread(Linux): addEventMonitorPath: %@ -- %@", 
...
- (void)_removePath:(NSString *)absolutePath
{
  ...
  if (in_fd < 0)
    {
      NSLog(@"NXFileSystemMonitorThread(Linux): ERROR trying to remove path without opened kernel queue!");
      return;
    }

  if (absolutePath == nil || [absolutePath isEqualToString:@""])
    {
      NSLog(@"NXFileSystemMonitorThread(Linux): ERROR trying to remove empty path!");
      return;
    }
  ...
  if ((pathDict = [_pathFDList objectForKey:absolutePath]) == nil)
    {
      NSLog(@"NXFileSystemMonitorThread(Linux): ERROR no info for monitor at path %@ found!", absolutePath);
      return;
    }
  path_fd = [[pathDict objectForKey:@"Descriptor"] intValue];
...
{
  NSDebugLLog(@"NXFileSystemMonitor",
              @"NXFileSystemMonitorThread(Linux): startEventMonitorThread: "
...
{
  NSDebugLLog(@"NXFileSystemMonitor",
              @"NXFileSystemMonitorThread(Linux): stopEventMonitorThread: "
              "inotify descriptor %i", in_fd);
...
- (void)checkForEvents
{
  ...
  // NSLog(@"NXFileSystemMonitorThread [checkForFSEvents]: checking for events...");
