      ...
      if (!sbit->buffer) {
        if (!delta_flags) {
          ...
          if (delta_flags & 0x4) {
            ...
            y += /*sbit->yadvance +*/ (ts.m22 < 0) ? delta_data[1] : -delta_data[1];
...
- (NSSize)advancementForGlyph:(NSGlyph)glyph
{
  ...
  if (glyph == NSControlGlyph || glyph == GSAttachmentGlyph)
    return NSZeroSize;
...
- (NSRect)boundingRectForGlyph:(NSGlyph)glyph
{
  ...
  return NSMakeRect(bbox.xMin / 64.0, bbox.yMin / 64.0, (bbox.xMax - bbox.xMin) / 64.0,
                    (bbox.yMax - bbox.yMin) / 64.0);
}

- (NSPoint)positionOfGlyph:(NSGlyph)g precededByGlyph:(NSGlyph)prev isNominal:(BOOL *)nominal
{
  NSPoint a;
  FT_Face face;
  FT_Size size;
...
static int bezierpath_cubic_to(const FT_Vector *c1, const FT_Vector *c2, const FT_Vector *to,
                               void *user)
{
  NSBezierPath *path = (NSBezierPath *)user;
  NSPoint b, c, d;
