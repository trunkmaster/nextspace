...
void set_font_options(cairo_font_options_t *options)
{
  ...
  if ((subpixel = [ud integerForKey:@"GSFontAntiAlias"])) {
...
- (BOOL)setupAttributes
{
   ...
   */
  cairo_matrix_init(&font_matrix, matrix[0], matrix[1], -matrix[2], matrix[3], matrix[4],
                    matrix[5]);
...
static BOOL _cairo_extents_for_NSGlyph(cairo_scaled_font_t *scaled_font, NSGlyph glyph,
...
- (void)drawGlyphs:(const NSGlyph *)glyphs length:(int)length on:(cairo_t *)ct
{
  ...
  if (cairo_status(ct) != CAIRO_STATUS_SUCCESS) {
    NSLog(@"Error drawing string: '%s' for string %s", cairo_status_to_string(cairo_status(ct)),
          str);
  }
}
