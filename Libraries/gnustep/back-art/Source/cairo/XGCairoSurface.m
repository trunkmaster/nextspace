...
- (id)initWithDevice:(void *)device
{
  ...
  */
  ...
  _surface = cairo_xlib_surface_create(dpy, drawable, visual, GSWINDEVICE->xframe.size.width,
...
- (NSImage *)contentsOfScreen:(int)screen inRect:(NSRect)rect
{
  ...
  if (XGetWindowAttributes(dpy, win, &attrs)) {
    ...
    dest = cairo_image_surface_create_for_data([bmp bitmapData], CAIRO_FORMAT_ARGB32, width, height,
                                               [bmp bytesPerRow]);
