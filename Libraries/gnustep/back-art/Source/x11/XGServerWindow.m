...
#define WINDOW_WITH_TAG(windowNumber) \
  (gswindow_device_t *)NSMapGet(windowtags, (void *)(uintptr_t)windowNumber)
                                           ...
                                           int format, int count, int *retCount)
{
  ...
  if ((type != AnyPropertyType && type != type_ret) ||
      ...
      (bytes_after_ret != 0) || (format != 0 && format != fmt_ret)) {
    NSLog(@"XGetWindowProperty type %lu type_ret %lu count %d count_ret %lu "
          @"format %d format_ret %d bytes_after_ret %lu",
...
static void setNormalHints(Display *d, gswindow_device_t *w)
{
  if (w->siz_hints.flags & (USPosition | PPosition))
    NSDebugLLog(@"XGTrace", @"Hint posn %lu: %d, %d", w->number, w->siz_hints.x,
                w->siz_hints.y);
  if (w->siz_hints.flags & (USSize | PSize))
    NSDebugLLog(@"XGTrace", @"Hint size %lu: %d, %d", w->number,
                w->siz_hints.width, w->siz_hints.height);
  if (w->siz_hints.flags & PMinSize)
    NSDebugLLog(@"XGTrace", @"Hint mins %lu: %d, %d", w->number,
                w->siz_hints.min_width, w->siz_hints.min_height);
  if (w->siz_hints.flags & PMaxSize)
    NSDebugLLog(@"XGTrace", @"Hint maxs %lu: %d, %d", w->number,
                w->siz_hints.max_width, w->siz_hints.max_height);
  if (w->siz_hints.flags & PResizeInc)
    NSDebugLLog(@"XGTrace", @"Hint incr %lu: %d, %d", w->number,
                w->siz_hints.width_inc, w->siz_hints.height_inc);
 ...
 * only once.
 */
static void setWindowHintsForStyle(Display *dpy, Window window, unsigned int styleMask,
                                   Atom mwhints_atom)
{
  MwmHints *hints;
  ...
  success = XGetWindowProperty(
      dpy, window, mwhints_atom, 0, sizeof(MwmHints) / sizeof(unsigned long),
      False, AnyPropertyType, &type_ret, &format_ret, &nitems_ret,
      &bytes_after_ret, (unsigned char **)&hints);
  ...
  if (styleMask == NSBorderlessWindowMask || !handlesWindowDecorations) {
    hints->flags |= MWM_HINTS_DECORATIONS;
    hints->flags |= MWM_HINTS_FUNCTIONS;
    hints->decorations = 0;
    hints->functions = 0;
  ...
  /* Set the hints */
  XChangeProperty(dpy, window, mwhints_atom, mwhints_atom, 32, PropModeReplace,
                  (unsigned char *)hints,
...
BOOL AtomPresentAndPointsToItself(Display *dpy, Atom atom, Atom type)
{
  ...
  Window *win =
      (Window *)PropGetCheckProperty(dpy, root, atom, type, 32, -1, &count);
  if (win != 0) {
    Window *win1 =
        (Window *)PropGetCheckProperty(dpy, *win, atom, type, 32, -1, &count);
...
- (NSRect) _XWinRectToOSWinRect: (NSRect)x for: (void*)window
{
  ...
  NSDebugLLog(@"Frame", @"XW2OW %@ %@", NSStringFromRect(x),
              NSStringFromRect(o));
  return o;
}
...
- (BOOL)_checkWMSupports:(Atom)feature {
  ...
  data = (Atom *)PropGetCheckProperty(dpy, root, generic._NET_SUPPORTED_ATOM,
                                      XA_ATOM, 32, -1, &count);
  if (data != 0) {
    int i = 0;
...
Bool _get_next_prop_new_event(Display *display, XEvent *event, char *arg) {
  ...
  if (event->type == PropertyNotify && event->xproperty.window == data[0] &&
...
- (gswindow_device_t *)_createServerWindow:(NSRect)frame
  ...
  window->ident = XCreateWindow(
      dpy, window->root, NSMinX(frame), NSMinY(frame), NSWidth(frame),
      NSHeight(frame), 0, context->depth, CopyFromParent, context->visual,
      // Don't set the CWBackPixel, as the background of the
      // window may be different.
      (CWColormap | CWBorderPixel | CWOverrideRedirect), &window->xwn_attrs);
  ...
  if (screen >= 0) {
    ...
    XChangeProperty(dpy, window->ident,
                    generic._NET_WM_SYNC_REQUEST_COUNTER_ATOM, XA_CARDINAL, 32,
                    PropModeReplace,
                    (unsigned char *)&(window->net_wm_sync_request_counter), 1);
...
- (unsigned long *)_getNetFrameExtents:(Window)win {
   ...
   */
  extents = (unsigned long *)PropGetCheckProperty(
      dpy, win, generic._NET_FRAME_EXTENTS_ATOM, XA_CARDINAL, 32, 4, &count);
  if (extents != 0) {
...
- (BOOL)_checkStyle:(unsigned)style {
  ...
  if (extents != 0) {
    ...
    if (repp != window->root) {
      ...
      */
      if (generic.flags.doubleParentWindow == YES || (repx == 0 && repy == 0)) {
        ...
        while (new_parent && (new_parent != window->root)) {
          ...
          if (!XQueryTree(dpy, parent, &root, &new_parent, &children,
                          &nchildren)) {
            new_parent = None;
            if (children) {
      ...
      if ((l >= 0) && (r >= 0) && (t >= 0) && (b >= 0)) {
        ...
        NSLog(@"Reparenting resulted in negative border %d, %d, %d, %d", l, r,
              t, b);
      }
    }
  }
...
- (NSString *)windowManagerName {
  if (generic.wm & XGWM_WINDOWMAKER) {
    ...
    int count;
    Window *win = (Window *)PropGetCheckProperty(
        dpy, DefaultRootWindow(dpy), generic._NET_SUPPORTING_WM_CHECK_ATOM,
        XA_WINDOW, 32, -1, &count);
    char *name =
        (char *)PropGetCheckProperty(dpy, *win, generic._NET_WM_NAME_ATOM,
                                     generic.UTF8_STRING_ATOM, 0, 0, &count);
    if (name) {
...
- (XGWMProtocols)_checkWindowManager {
  ...
  data = (Atom *)PropGetCheckProperty(dpy, root,
                                      generic._WINDOWMAKER_WM_PROTOCOLS_ATOM,
                                      XA_ATOM, 32, -1, &count);
  if (data != 0) {
    int i = 0;
    ...
    if (i < count) {
      if (AtomPresentAndPointsToItself(
              dpy, generic._WINDOWMAKER_NOTICEBOARD_ATOM, XA_WINDOW)) {
  ...
  /* Now check for Gnome */
  if (AtomPresentAndPointsToItself(dpy, generic._WIN_SUPPORTING_WM_CHECK_ATOM,
                                   XA_CARDINAL)) {
    wmflags |= XGWM_GNOME;
  }

  /* Now check for NET (EWMH) compliant window manager */
  if (AtomPresentAndPointsToItself(dpy, generic._NET_SUPPORTING_WM_CHECK_ATOM,
                                   XA_WINDOW)) {
    wmflags |= XGWM_EWMH;
  }

  NSDebugLLog(@"WM", @"WM Protocols: WindowMaker=(%s) GNOME=(%s) EWMH=(%s)",
...
- (gswindow_device_t *)_rootWindow {
  ...
  window->monitor_id = 0;
  if (window->ident) {
    XGetGeometry(dpy, window->ident, &window->root, &x, &y, &width, &height,
...
- (void)_setupMouse {
  ...
  unsigned char buttons[7] = {Button1, Button2, Button3, Button4,
                              Button5, 6,       7};
  int masks[5] = {Button1Mask, Button2Mask, Button3Mask, Button4Mask,
                  Button5Mask};
  /*
  ...
  if (numButtons > 7) {
    NSDebugLLog(@"XGTrace",
                @"Warning - mouse/pointer seems to have more than 7 buttons "
                @"(%d) - just using one to seven",
                numButtons);
    numButtons = 7;
  }
...
- (void)_setupRootWindow {
  ...
  XInternAtoms(dpy, atom_names, sizeof(atom_names) / sizeof(char *), False,
               generic.atoms);
#else
  {
    int atomCount;

    for (atomCount = 0; atomCount < sizeof(atom_names) / sizeof(char *);
         atomCount++)
   ...
   */
  ROOT =
      XCreateSimpleWindow(dpy, RootWindow(dpy, defScreen), 0, 0, 1, 1, 0, 0, 0);
  ...
  // Always send GNUstepWMAttributes
  {
     ...
     * hold 64bit values.
     */
    XChangeProperty(dpy, ROOT, generic._GNUSTEP_WM_ATTR_ATOM,
                    generic._GNUSTEP_WM_ATTR_ATOM, 32, PropModeReplace,
                    (unsigned char *)&win_attrs,
                    sizeof(GNUstepWMAttributes) / sizeof(CARD32));
  }
  ...
  if ((generic.wm & XGWM_EWMH) != 0) {
     ...
     * hold 64bit values.
     */
    XChangeProperty(dpy, ROOT, generic._NET_WM_PID_ATOM, XA_CARDINAL, 32,
  ...
  if (handlesWindowDecorations == YES) {
    ...
    if ([defs boolForKey:@"GSIgnoreRootOffsets"] == YES) {
      ...
      offsets = (uint16_t *)PropGetCheckProperty(
          dpy, DefaultRootWindow(dpy), generic._GNUSTEP_FRAME_OFFSETS_ATOM,
...
- (void)_createBuffer:(gswindow_device_t *)window {
  ...
  window->buffer = XCreatePixmap(dpy, window->root, NSWidth(window->xframe),
                                 NSHeight(window->xframe), window->depth);
  ...
  XFillRectangle(dpy, window->buffer, window->gc, 0, 0, NSWidth(window->xframe),
...
- (void)_setNetWMIconFor:(Window)window {
  ...
  if (iconPropertyData != 0) {
    XChangeProperty(dpy, window, generic._NET_WM_ICON_ATOM, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)iconPropertyData,
                    iconSize);
  }
}
...
- (void)styleoffsets:(float *)l :(float *)r :(float *)t :(float *)b :(unsigned int)style :(Window)win
{
  ...
  if (win && ((generic.wm & XGWM_EWMH) != 0)) {
    ...
    if (extents) {
      NSDebugLLog(@"Frame",
                  @"Window %lu, left %lu, right %lu, top %lu, bottom %lu", win,
                  extents[0], extents[1], extents[2], extents[3]);
      *l = extents[0];
      *r = extents[1];
      *t = extents[2];
  ...
  */
  ...
  if (o->known == YES) {
    ...
    *b = o->b;
    *t = o->t;
    NSDebugLLog(@"Frame", @"Window %lu, offsets %f, %f, %f, %f", win, *l, *r,
                *t, *b);
    return;
  }
  ...
  if ((generic.wm & XGWM_WINDOWMAKER) != 0) {
    *l = *r = *t = *b = 1.0;
    ...
    NSDebugLLog(@"Frame", @"Window %lu, windowmaker %f, %f, %f, %f", win, *l,
                *r, *t, *b);
    ...
    *l = *r = *t = *b = 4;
    ...
    NSDebugLLog(@"Frame", @"Window %lu, EWMH %f, %f, %f, %f", win, *l, *r, *t,
                *b);
  } else {
     ...
     */
    *l = *r = *t = *b = 0.0;
    NSDebugLLog(@"Frame", @"Window %lu, unknown %f, %f, %f, %f", win, *l, *r,
                *t, *b);
  }
}

- (void)stylewindow:(unsigned int)style
                   :(int)win {
  gswindow_device_t *window;
...
- (void)titlewindow:(NSString *)window_title :(int)win {
  ...
  if (window_title && window->ident) {
    ...
    error = Xutf8TextListToTextProperty(dpy, (char **)&title, 1, XUTF8StringStyle, &window_name);
#endif
    if (error != Success) {
      title = [window_title lossyCString];
      XStringListToTextProperty((char **)&title, 1, &window_name);
    }
    XSetWMName(dpy, window->ident, &window_name);
    XSetWMIconName(dpy, window->ident, &window_name);
    XFree(window_name.value);

    /* Set _NET_WM_NAME and _NET_WM_ICON_NAME */
    XChangeProperty(dpy, window->ident, generic._NET_WM_NAME_ATOM, generic.UTF8_STRING_ATOM, 8,
                    PropModeReplace, (unsigned char *)name, strlen(name));
    XChangeProperty(dpy, window->ident, generic._NET_WM_ICON_NAME_ATOM, generic.UTF8_STRING_ATOM, 8,
                    PropModeReplace, (unsigned char *)name, strlen(name));
  }
}
...
- (void)miniwindow:(int)win {
   ...
   */
  if (generic.flags.appOwnsMiniwindow && (window->gen_hints.flags & IconWindowHint) == 0) {
    NSWindow *nswin;
...
- (void)setWindowdevice:(int)win forContext:(NSGraphicsContext *)ctxt {
  ...
  [self styleoffsets:&l :&r :&t :&b :window->win_attrs.window_style :window->ident];
...
Pixmap alphaMaskForImage(Display *xdpy, Drawable draw,
                         const unsigned char *data, int w, int h, int colors,
                         unsigned int alpha_treshold) {
  Pixmap pix;
  ...
  pix = XCreatePixmapFromBitmapData(xdpy, draw, (char *)aData, w, h, 1L, 0L, 1);
  free(aData);
  return pix;
}
...
void swapColors(unsigned char *image_data, NSBitmapImageRep *rep) {
...
- (int)_createAppIconPixmaps {
  ...
  xIconPixmap =
      XCreatePixmap(dpy, rcontext->drawable, width, height, rcontext->depth);
  XPutImage(dpy, xIconPixmap, rcontext->copy_gc, rxImage->image, 0, 0, 0, 0,
            width, height);
  RDestroyXImage(rcontext, rxImage);

  xIconMask =
      alphaMaskForImage(dpy, ROOT, [rep bitmapData], width, height, colors, 0);
...
- (void)orderwindow:(int)op :(int)otherWin :(int)winNum {
  ...
  NSDebugLLog(@"XGTrace", @"DPSorderwindow: %d : %d : %d", op, otherWin,
              winNum);
  ...
  case NSWindowBelow:
    if (other != 0) {
      ...
      XWindowChanges chg;
      chg.stack_mode = Below;
      XReconfigureWMWindow(dpy, window->ident, window->screen_id, CWStackMode,
                           &chg);
    }
  ...
  if ([[image backgroundColor] alphaComponent] * 256 <= ALPHA_THRESHOLD) {
    ...
    if (rep != nil) {
      if ([rep samplesPerPixel] == 4) {
        pixmap = alphaMaskForImage(dpy, GET_XDRAWABLE(window), [rep bitmapData],
                                   [rep pixelsWide], [rep pixelsHigh],
                                   [rep samplesPerPixel], ALPHA_THRESHOLD);
      }
    }
  }

  XShapeCombineMask(dpy, window->ident, ShapeBounding, 0, 0, pixmap, ShapeSet);
...
- (void)movewindow:(NSPoint)loc :(int)win {
  ...
  window->siz_hints.y =
      (int)(xScreenSize.height - loc.y - window->siz_hints.height);
  XMoveWindow(dpy, window->ident, window->siz_hints.x, window->siz_hints.y);
  setNormalHints(dpy, window);
}
...
- (void)placewindow:(NSRect)rect :(int)win {
  ...
  XMoveResizeWindow(dpy, window->ident, window->siz_hints.x,
                    window->siz_hints.y, window->siz_hints.width,
  ...
  if ((int)(window->win_attrs.window_level) != level ||
    ...
    if ((generic.wm & XGWM_EWMH) != 0) {
      ...
      if (level == NSModalPanelWindowLevel) {
        ...
        skipTaskbar = YES;
      } else if (level == NSDockWindowLevel || level == NSStatusWindowLevel) {
        data[0] = generic._NET_WM_WINDOW_TYPE_DOCK_ATOM;
        skipTaskbar = YES;
      }
      ...
      XChangeProperty(dpy, window->ident, generic._NET_WM_WINDOW_TYPE_ATOM,
                      XA_ATOM, 32, PropModeReplace, (unsigned char *)&data,
                      len);
      ...
      XChangeProperty(dpy, window->ident, generic._WIN_LAYER_ATOM, XA_CARDINAL,
                      32, PropModeReplace, (unsigned char *)&flag, 1);
...
- (NSArray *)windowlist {
  ...
  windowOrder = (Window *)PropGetCheckProperty(
      dpy, rootWindow->ident, generic._NET_CLIENT_LIST_STACKING_ATOM, XA_WINDOW,
      32, -1, &c);
...
- (void)_addExposedRectangle:(XRectangle)rectangle :(int)win :(BOOL)ignoreBacking {
  ...
  if (!ignoreBacking && window->type != NSBackingStoreNonretained) {
    ...
    if ((window->gdriverProtocol & GDriverHandlesExpose)) {
      ...
      NSRect rect = NSMakeRect(rectangle.x, rectangle.y, rectangle.width,
                               rectangle.height);
      [[GSCurrentContext() class] handleExposeRect:rect
                                         forDriver:window->gdriver];
    } else {
      XCopyArea(dpy, window->buffer, window->ident, window->gc, rectangle.x,
                rectangle.y, rectangle.width, rectangle.height, rectangle.x,
                rectangle.y);
    }
  } else {
    NSRect rect;
...
- (void)flushwindowrect:(NSRect)rect :(int)win {
  ...
  if (width > 0 || height > 0) {
    [[self class] waitAllContexts];
    if ((window->gdriverProtocol & GDriverHandlesBacking)) {
      NSDebugLLog(@"XGFlush", @"expose X rect ((%d, %d), (%d, %d))", xi, yi,
                  width, height);
      /* Temporary protocol until we standardize the backing buffer */
      [[GSCurrentContext() class] handleExposeRect:rect
                                         forDriver:window->gdriver];
    } else {
      NSDebugLLog(@"XGFlush", @"copy X rect ((%d, %d), (%d, %d))", xi, yi,
                  width, height);

      XCopyArea(dpy, window->buffer, window->ident, window->gc, xi, yi, width,
                height, xi, yi);
    }
  }
  ...
  if (window->net_wm_sync_request_counter_value_low != 0 ||
      window->net_wm_sync_request_counter_value_high != 0) {
    XSyncValue value;
    XSyncIntsToValue(&value, window->net_wm_sync_request_counter_value_low,
...
- (void)setMouseLocation:(NSPoint)mouseLocation onScreen:(int)aScreen {
  ...
  XWarpPointer(dpy, None, [self xDisplayRootWindow], 0, 0, 0, 0, destX, destY);
}
...
- (void)setinputfocus:(int)win {
  ...
  if ((generic.wm & XGWM_EWMH) != 0) {
    ...
    NSDebugLLog(@"Focus", @"Setting user time for %lu to %lu", window->ident,
                last);
    XChangeProperty(dpy, window->ident, generic._NET_WM_USER_TIME_ATOM,
                    XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&last,
                    1);
  }
...
- (void)setalpha:(float)alpha :(int)win {
  ...
  if (alpha == 1.0) {
    XDeleteProperty(window->display, window->ident,
                    generic._NET_WM_WINDOW_OPACITY_ATOM);
  } else {
    unsigned int opacity = (unsigned int)(alpha * 0xffffffffU);
    XChangeProperty(window->display, window->ident,
                    generic._NET_WM_WINDOW_OPACITY_ATOM, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&opacity, 1L);
    if (window->parent != window->root) {
      XChangeProperty(window->display, window->parent,
                      generic._NET_WM_WINDOW_OPACITY_ATOM, XA_CARDINAL, 32,
...
- (Cursor)_blankCursor {
  if (xgps_blank_cursor == None) {
    ...
    shape = XCreatePixmapFromBitmapData(dpy, drw, xgps_blank_cursor_bits, 16,
                                        16, 1, 0, 1);
    mask = XCreatePixmapFromBitmapData(dpy, drw, xgps_blank_cursor_bits, 16, 16,
                                       1, 0, 1);
    ...
    xgps_blank_cursor =
        XCreatePixmapCursor(dpy, shape, mask, &white, &black, 0, 0);
...
Pixmap xgps_cursor_image(Display *xdpy, Drawable draw,
                         const unsigned char *data, int w, int h, int colors,
                         XColor *fg, XColor *bg) {
  int j, i, min, max;
  Pixmap pix;
...
- (void)imagecursor:(NSPoint)hotp :(NSImage *)image :(void **)cid {
...
#else // !HAVE_XCURSOR
  {
    ...
    cursor = XCreatePixmapCursor(dpy, source, mask, &fg, &bg, (int)hotp.x,
                                 (int)hotp.y);
...
static NSSize _screenSize(Display *dpy, int screen) {
  int x, y;
  unsigned int width, height, border_width, depth;
  Window root_window;
  NSSize scrSize;

  XGetGeometry(dpy, RootWindow(dpy, screen), &root_window, &x, &y, &width,
  ...
  if (screen_res != NULL) {
    ...
    if (monitorsCount != 0) {
      ...
      NSMutableArray *tmpScreens =
          [NSMutableArray arrayWithCapacity:monitorsCount];
      RROutput primary_output = XRRGetOutputPrimary(dpy, root);

      monitors =
          NSZoneMalloc([self zone], monitorsCount * sizeof(MonitorDevice));
...
- (NSImage *)iconTileImage {
  ...
  pwin = (Window *)PropGetCheckProperty(dpy, win,
                                        generic._WINDOWMAKER_NOTICEBOARD_ATOM,
                                        XA_WINDOW, 32, -1, &count);
  if (pwin == NULL)
    return [super iconTileImage];

  tile = PropGetCheckProperty(dpy, *pwin, generic._WINDOWMAKER_ICON_TILE_ATOM,
                              generic._RGBA_IMAGE_ATOM, 8, -1, &count);
  XFree(pwin);
...
- (NSSize)iconSize {
  ...
  status =
      XGetIconSizes(dpy, DefaultRootWindow(dpy), &xiconsize, &count_return);
  if (status) {
    if ((generic.wm & XGWM_WINDOWMAKER) != 0) {
      /* must add 4 pixels for the border which windowmaker leaves out
         but gnustep draws over. */
      iconSize =
          NSMakeSize(xiconsize[0].max_width + 4, xiconsize[0].max_height + 4);
    } else {
      iconSize = NSMakeSize(xiconsize[0].max_width, xiconsize[0].max_height);
    }
...
- (unsigned int)numberOfDesktops:(int)screen {
  ...
  num = (unsigned int *)PropGetCheckProperty(
      dpy, RootWindow(dpy, screen), generic._NET_NUMBER_OF_DESKTOPS_ATOM,
      XA_CARDINAL, 32, 1, &c);
...
- (NSArray *)namesOfDesktops:(int)screen {
  ...
  names = (char *)PropGetCheckProperty(dpy, RootWindow(dpy, screen),
                                       generic._NET_DESKTOP_NAMES_ATOM,
                                       generic.UTF8_STRING_ATOM, 0, 0, &c);
  if (names) {
...
- (unsigned int)desktopNumberForScreen:(int)screen {
  ...
  num = (unsigned int *)PropGetCheckProperty(dpy, RootWindow(dpy, screen),
                                             generic._NET_CURRENT_DESKTOP_ATOM,
                                             XA_CARDINAL, 32, 1, &c);
...
- (unsigned int)desktopNumberForWindow:(int)win {
  ...
  num = (unsigned int *)PropGetCheckProperty(
      dpy, window->ident, generic._NET_WM_DESKTOP_ATOM, XA_CARDINAL, 32, 1, &c);
...
- (void)setShadow:(BOOL)hasShadow :(int)win {
  ...
  if (hasShadow) {
    ...
    XChangeProperty(window->display, window->ident,
                    generic._NET_WM_WINDOW_SHADOW_ATOM, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&shadow, 1L);
    if (window->parent != window->root) {
      XChangeProperty(window->display, window->parent,
                      generic._NET_WM_WINDOW_SHADOW_ATOM, XA_CARDINAL, 32,
                      PropModeReplace, (unsigned char *)&shadow, 1);
    }
  } else {
    XDeleteProperty(window->display, window->ident,
                    generic._NET_WM_WINDOW_SHADOW_ATOM);
    if (window->parent != window->root) {
      XDeleteProperty(window->display, window->parent,
                      generic._NET_WM_WINDOW_SHADOW_ATOM);
    }
  }
}
...
- (BOOL)_ewmh_isHidden:(Window)win {
  ...
  data = (Atom *)PropGetCheckProperty(dpy, win, generic._NET_WM_STATE_ATOM,
                                      XA_ATOM, 32, -1, &count);
...
- (void)setIgnoreMouse:(BOOL)ignoreMouse :(int)win {
  ...
  XFixesSetWindowShapeRegion(dpy, window->ident, ShapeInput, 0, 0, region);
  if (region != None) {
