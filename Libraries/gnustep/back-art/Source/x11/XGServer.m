...
static NSString *_parse_display_name(NSString *name, int *dn, int *sn)
{
  int d, s;
  NSString *host;
  NSArray *a;
...
- (id)_initXContext
{
  ...
  NSDebugLog(@"Opened display %@, display %d screen %d", display_name, display_id, screen_id);
  [server_info setObject:display_name forKey:GSDisplayName];
  [server_info setObject:[NSNumber numberWithInt:display_id] forKey:GSDisplayNumber];
  [server_info setObject:[NSNumber numberWithInt:screen_id] forKey:GSScreenNumber];

  /* Setup screen*/
  if (screenList == NULL)
    screenList = NSCreateMapTable(NSIntMapKeyCallBacks, NSObjectMapValueCallBacks, 20);
  ...
  inputServer = [[XGInputServer allocWithZone:[self zone]] initWithDelegate:nil
                                                                     display:dpy
                                                                        name:@"XIM"];
...
- (XGScreenContext *)_defaultScreenContext
{
  ...
  if (screenCtxt == NULL) {
    screenCtxt = [[XGScreenContext alloc] initForDisplay:dpy screen:defScreen];
...
static unsigned int flip_bytes16(unsigned int i) { return ((i >> 8) & 0xff) | ((i << 8) & 0xff00); }
 ...
 */
- (void)getForScreen:(int)screen_id
...
{
  if ([[GSCurrentContext() class] respondsToSelector:@selector(waitAllContexts)])
...
{
  return inputServer ? [(XGInputServer *)inputServer inputMethodStyle] : (NSString *)nil;
}

- (NSString *)fontSize:(int *)size
{
  return inputServer ? [(XGInputServer *)inputServer fontSize:size] : (NSString *)nil;
}

- (BOOL)clientWindowRect:(NSRect *)rect
{
  return inputServer ? [(XGInputServer *)inputServer clientWindowRect:rect] : NO;
}

- (BOOL)statusArea:(NSRect *)rect
{
  return inputServer ? [(XGInputServer *)inputServer statusArea:rect] : NO;
}

- (BOOL)preeditArea:(NSRect *)rect
{
  return inputServer ? [(XGInputServer *)inputServer preeditArea:rect] : NO;
}

- (BOOL)preeditSpot:(NSPoint *)p
{
  return inputServer ? [(XGInputServer *)inputServer preeditSpot:p] : NO;
}

- (BOOL)setStatusArea:(NSRect *)rect
{
  return inputServer ? [(XGInputServer *)inputServer setStatusArea:rect] : NO;
}

- (BOOL)setPreeditArea:(NSRect *)rect
{
  return inputServer ? [(XGInputServer *)inputServer setPreeditArea:rect] : NO;
}

- (BOOL)setPreeditSpot:(NSPoint *)p
{
  return inputServer ? [(XGInputServer *)inputServer setPreeditSpot:p] : NO;
}
...
- (void)_updateInputMethodState
{
  ...
  if ([[displayServer inputMethodStyle] isEqual:@"OverTheSpot"]) {
    ...
    preedit_area.size.width = frame.origin.x + frame.size.width - preedit_area.origin.x;
    preedit_area.size.height = status_area.size.height;
