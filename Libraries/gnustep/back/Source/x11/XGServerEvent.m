...
static NSEvent *process_key_event(XEvent *xEvent, XGServer *ctxt, NSEventType eventType,
...
- (int)XGErrorHandler:(Display *)display :(XErrorEvent *)err
{
  ...
  if (err->type == 0 && GSDebugSet(@"XSynchronize") == NO) {
    NSLog(@"X-Windows error - %s\n\
       ...
       "
          @"serial number: %lu\n\
        request code: %d\n",
          buffer, XDisplayName(DisplayString(display)), err->type, err->serial, err->request_code);
    return 0;
  }
  [NSException raise:NSWindowServerCommunicationException
       ...
       serial number: %lu\n\
        request code: %d\n",
                     buffer, XDisplayName(DisplayString(display)), err->type, err->serial,
                     err->request_code];
  return 0;
}
...
- (NSPoint)_XPointToOSPoint:(NSPoint)x for:(void*)window
{
  ...
  NSDebugLLog(@"Frame", @"X2OP %lu, %x, %@, %@", win->number, style, NSStringFromPoint(x),
              NSStringFromPoint(o));
  return o;
}
  ...
  switch (xEvent.type) {
    ...
    case ButtonPress:
      NSDebugLLog(@"NSEvent",
                  ...
                  xEvent.xbutton.window, xEvent.xbutton.time, generic.lastClick);
      {
        /*
         ...
         */
        {
          ...
          if (xEvent.xbutton.time >= (unsigned long)(generic.lastClick + clickTime))
            incrementCount = NO;
        ...
        } else if (xEvent.xbutton.button == generic.rMouse && generic.rMouse != 0) {
          if (swapMouseButtons) {
            eventType = NSLeftMouseDown;
            buttonNumber = generic.lMouse;
          } else {
        ...
        } else if (xEvent.xbutton.button == generic.mMouse && generic.mMouse != 0) {
          eventType = NSOtherMouseDown;
          buttonNumber = generic.mMouse;
        } else if (xEvent.xbutton.button == generic.upMouse && generic.upMouse != 0) {
          deltaY = 1. * mouseScrollMultiplier;
          eventType = NSScrollWheel;
          buttonNumber = generic.upMouse;
        } else if (xEvent.xbutton.button == generic.downMouse && generic.downMouse != 0) {
          deltaY = -1. * mouseScrollMultiplier;
          eventType = NSScrollWheel;
        ...
        if (generic.flags.useWindowMakerIcons == 1) {
           ...
           * to Window Maker.
           */
          if ((cWin->win_attrs.window_style & (NSMiniWindowMask | NSIconWindowMask)) != 0 &&
              eventType == NSLeftMouseDown) {
            if (cWin->parent == None)
              break;
                  ...
                  XGetAtomName(dpy, xEvent.xclient.message_type));
      {
        ...
        if (xEvent.xclient.message_type == generic.WM_PROTOCOLS_ATOM) {
          ...
          if (wm_protocol == generic.WM_DELETE_WINDOW_ATOM) {
            ...
            e = [self _handleTakeFocusAtom:xEvent forContext:gcontext];
          ...
          if ([XGServer _windowForXWindow:source] == NULL) {
            [[XGDragView sharedDragView] setupDragInfoFromXEvent:&xEvent];
          }
          ...
          */
          XTranslateCoordinates(dpy, xEvent.xclient.window, RootWindow(dpy, cWin->screen_id), 0, 0,
                                &root_x, &root_y, &root_child);
          cWin->xframe.origin.x = root_x;
          cWin->xframe.origin.y = root_y;

          eventLocation.x = XDND_POSITION_ROOT_X(&xEvent) - NSMinX(cWin->xframe);
          eventLocation.y = XDND_POSITION_ROOT_Y(&xEvent) - NSMinY(cWin->xframe);
            eventLocation = [self _XPointToOSPoint: eventLocation
                                               for: cWin];
            ...
            if ([XGServer _windowForXWindow:source] == NULL) {
              [[XGDragView sharedDragView] updateDragInfoFromEvent:e];
            }
    ...
    case ConfigureNotify:
      NSDebugLLog(@"NSEvent",
                  @"%lu ConfigureNotify "
                  @"x:%d y:%d w:%d h:%d b:%d %c",
                  xEvent.xconfigure.window, xEvent.xconfigure.x, xEvent.xconfigure.y,
                  xEvent.xconfigure.width, xEvent.xconfigure.height, xEvent.xconfigure.border_width,
                  xEvent.xconfigure.send_event ? 'T' : 'F');
      {
        ...
        if (cWin != 0) {
          ...
          x = NSMakeRect(xEvent.xconfigure.x, xEvent.xconfigure.y, xEvent.xconfigure.width,
          ...
          */
          if (xEvent.xconfigure.send_event == 0) {
            int root_x, root_y;
            Window root_child;
            XTranslateCoordinates(dpy, xEvent.xconfigure.window, RootWindow(dpy, cWin->screen_id),
          ...
          cWin->xframe = x;
            ...
            NSDebugLLog(@"Moving", @"Update win %lu:\n   original:%@\n   new:%@", cWin->number,
                        NSStringFromRect(r), NSStringFromRect(x));
            /*
    ...
    case Expose:
      NSDebugLLog(@"NSEvent", @"%lu Expose\n", xEvent.xexpose.window);
      {
        ...
        if (cWin != 0) {
          ...
          rect = NSMakeRect(rectangle.x, rectangle.y, rectangle.width, rectangle.height);
    ...
    case FocusOut:
      NSDebugLLog(@"Focus", @"%lu FocusOut (mode: %s - %i)\n", xEvent.xfocus.window,
                  (xEvent.xfocus.mode == NotifyNormal) ? "normal" : "specific", xEvent.xfocus.mode);
      {
        Window fw;
        int rev;
        ...
        NSDebugLLog(@"Focus", @"Focus went to %lu from %lu (xwin %lu)\n",
                    (nswin != nil) ? cWin->number : 0, generic.currentFocusWindow, fw);
      ...
      NSDebugLLog(@"NSEvent", @"%lu KeyRelease\n", xEvent.xkey.window);
      {
        ...
        */
        if (XEventsQueued(dpy, QueuedAfterReading)) {
          ...
          if (nev.type == KeyPress && nev.xkey.window == xEvent.xkey.window &&
              nev.xkey.time == xEvent.xkey.time && nev.xkey.keycode == xEvent.xkey.keycode) {
    ...
    case KeymapNotify:
      NSDebugLLog(@"NSEvent", @"%lu KeymapNotify\n", xEvent.xkeymap.window);
      {
        ...
        _control_pressed = 0;
        if ((_control_keysyms[0] != NoSymbol) && check_modifier(&xEvent, _control_keysyms[0])) {
          _control_pressed |= 1;
        }
        if ((_control_keysyms[1] != NoSymbol) && check_modifier(&xEvent, _control_keysyms[1])) {
          _control_pressed |= 2;
        }

        // Check if command is pressed
        _command_pressed = 0;
        if ((_command_keysyms[0] != NoSymbol) && check_modifier(&xEvent, _command_keysyms[0])) {
          _command_pressed |= 1;
        }
        if ((_command_keysyms[1] != NoSymbol) && check_modifier(&xEvent, _command_keysyms[1])) {
          _command_pressed |= 2;
        }

        // Check if alt is pressed
        _alt_pressed = 0;
        if ((_alt_keysyms[0] != NoSymbol) && check_modifier(&xEvent, _alt_keysyms[0])) {
          _alt_pressed |= 1;
        }
        if ((_alt_keysyms[1] != NoSymbol) && check_modifier(&xEvent, _alt_keysyms[1])) {
          _alt_pressed |= 2;
        }

        // Check if help is pressed
        _help_pressed = 0;
        if ((_help_keysyms[0] != NoSymbol) && check_modifier(&xEvent, _help_keysyms[0])) {
          _help_pressed |= 1;
        }
        if ((_help_keysyms[1] != NoSymbol) && check_modifier(&xEvent, _help_keysyms[1])) {
          _help_pressed |= 2;
        }
      }
      break;
      ...
      NSDebugLLog(@"NSEvent", @"%lu MapNotify\n", xEvent.xmap.window);
      {
        ...
        if (cWin != 0) {
          ...
          if (generic.desiredFocusWindow == cWin->number && generic.focusRequestNumber == 0) {
            NSDebugLLog(@"Focus", @"Refocusing %lu on map notify", cWin->number);
    ...
    case MotionNotify:
                  ...
                  xEvent.xmotion.x, xEvent.xmotion.y);
      {
        ...
        while (XPending(xEvent.xmotion.display)) {
          ...
          if (peek.type == MotionNotify && xEvent.xmotion.window == peek.xmotion.window &&
        ...
        xWin = (grabWindow == 0) ? xEvent.xmotion.window : grabWindow;
        if (cWin == 0 || xWin != cWin->ident)
          generic.cachedWindow = [XGServer _windowForXWindow:xWin];
        if (cWin == 0)
          break;
                  ...
                  XGetAtomName(dpy, xEvent.xproperty.atom));
      {
        if (xEvent.xproperty.atom == generic.WM_STATE_ATOM) {
          if (cWin == 0 || xEvent.xproperty.window != cWin->ident) {
            generic.cachedWindow = [XGServer _windowForXWindow:xEvent.xproperty.window];
          }
          if (cWin != 0) {
            int new_state;
            ...
            switch (new_state) {
              case IconicState:
                ...
                if ((generic.wm & XGWM_WINDOWMAKER) == 0 && cWin->wm_state == NormalState &&
              ...
              case NormalState:
                ...
                if (cWin->wm_state == IconicState &&
                  ...
                  NSDebugLLog(@"Miniaturize", @"%lu deminiaturized", cWin->number);
                  eventLocation = NSMakePoint(0, 0);
                  e = [NSEvent otherEventWithType:NSAppKitDefined
                                         location:eventLocation
                                    modifierFlags:0
          ...
          */
          if (generic.flags.doubleParentWindow) {
            ...
            while (new_parent && (new_parent != cWin->root)) {
              ...
              parent = new_parent;
              NSLog(@"QueryTree window is %lu (cwin root %lu)", parent, cWin->root);
              if (!XQueryTree(dpy, parent, &root, &new_parent, &children, &nchildren)) {
                new_parent = None;
                if (children) {
          ...
          if (o->known == NO) {
            ...
            if (l != o->l) {
              NSDebugLLog(@"NSEvent", @"Ignore left offset change from %d to %d\n", (int)o->l,
                          (int)l);
              changed = YES;
            }
            if (r != o->r) {
              NSDebugLLog(@"NSEvent", @"Ignore right offset change from %d to %d\n", (int)o->r,
                          (int)r);
              changed = YES;
            }
            if (t != o->t) {
              NSDebugLLog(@"NSEvent", @"Ignore top offset change from %d to %d\n", (int)o->t,
                          (int)t);
              changed = YES;
            }
            if (b != o->b) {
              NSDebugLLog(@"NSEvent", @"Ignore bottom offset change from %d to %d\n", (int)o->b,
                          (int)b);
              changed = YES;
            }
            if (changed == YES) {
    ...
    case SelectionRequest:
      NSDebugLLog(@"NSEvent", @"%lu SelectionRequest\n", xEvent.xselectionrequest.requestor);
      {
        ...
        if (data != nil) {
          ...
          xdnd_selection_send(&dnd, &xEvent.xselectionrequest, (unsigned char *)[data bytes],
                              [data length]);
        }
      }
      break;
    ...
    default:
      ...
      if (xEvent.type == XShmGetEventBase(dpy) + ShmCompletion &&
        ...
        [gcontext gotShmCompletion:((XShmCompletionEvent *)&xEvent)->drawable];
        break;
      }
#endif
#ifdef HAVE_XRANDR
...
- (NSEvent *)_handleTakeFocusAtom:(XEvent)xEvent forContext:(NSGraphicsContext *)gcontext
{
  ...
  NSDebugLLog(@"Focus", @"TakeFocus received by: %li (%lu) (focused = %lu, key = %d)", cWin->number,
              xEvent.xfocus.window, generic.currentFocusWindow, key_num);
  ...
  if ([NSApp isHidden]) {
    ...
    NSDebugLLog(@"Focus", @"Ignore transient application menu focus request.");
...
static void initialize_keyboard(void)
{
  ...
  // Initialize Control
  _control_keysyms[0] =
      key_sym_from_defaults(display, defaults, @"GSFirstControlKey", XK_Control_L);
...
static void set_up_num_lock(void)
{
  ...
  int i, j;
  unsigned int modifier_masks[8] = {ShiftMask, LockMask, ControlMask, Mod1Mask,
  ...
  for (j = 0; j < 8; j++)
    for (i = 0; i < (modifier_map->max_keypermod); i++) {
      if ((modifier_map->modifiermap)[i + j * modifier_map->max_keypermod] == _num_lock_keycode) {
...
static NSEvent *process_key_event(XEvent *xEvent, XGServer *context, NSEventType eventType,
                                  NSMutableArray *event_queue, BOOL keyRepeat)
{
  NSString *keys, *ukeys;
  KeySym keysym;
  ...
  modKeysym = (_mod_ignore_shift == YES) ? XLookupKeysym((XKeyEvent *)xEvent, 0) : keysym;
  ...
  NSDebugLLog(@"NSKeyEvent", @"keysym=%lu, keyCode=%d flags=%d (state=%d)", keysym, keyCode,
              eventFlags, ((XKeyEvent *)xEvent)->state);
  ...
  if (help_key) {
    ...
    xEvent->xkey.state = (xEvent->xkey.state & (ShiftMask | LockMask | _num_lock_mask));
...
#define XGPS_FUNCTIONKEY *eventModifierFlags = *eventModifierFlags | NSFunctionKeyMask;

    case XK_F1:
      XGPS_FUNCTIONKEY return NSF1FunctionKey;
    case XK_F2:
      XGPS_FUNCTIONKEY return NSF2FunctionKey;
    case XK_F3:
      XGPS_FUNCTIONKEY return NSF3FunctionKey;
    case XK_F4:
      XGPS_FUNCTIONKEY return NSF4FunctionKey;
    case XK_F5:
      XGPS_FUNCTIONKEY return NSF5FunctionKey;
    case XK_F6:
      XGPS_FUNCTIONKEY return NSF6FunctionKey;
    case XK_F7:
      XGPS_FUNCTIONKEY return NSF7FunctionKey;
    case XK_F8:
      XGPS_FUNCTIONKEY return NSF8FunctionKey;
    case XK_F9:
      XGPS_FUNCTIONKEY return NSF9FunctionKey;
    case XK_F10:
      XGPS_FUNCTIONKEY return NSF10FunctionKey;
    case XK_F11:
      XGPS_FUNCTIONKEY return NSF11FunctionKey;
    case XK_F12:
      XGPS_FUNCTIONKEY return NSF12FunctionKey;
    case XK_F13:
      XGPS_FUNCTIONKEY return NSF13FunctionKey;
    case XK_F14:
      XGPS_FUNCTIONKEY return NSF14FunctionKey;
    case XK_F15:
      XGPS_FUNCTIONKEY return NSF15FunctionKey;
    case XK_F16:
      XGPS_FUNCTIONKEY return NSF16FunctionKey;
    case XK_F17:
      XGPS_FUNCTIONKEY return NSF17FunctionKey;
    case XK_F18:
      XGPS_FUNCTIONKEY return NSF18FunctionKey;
    case XK_F19:
      XGPS_FUNCTIONKEY return NSF19FunctionKey;
    case XK_F20:
      XGPS_FUNCTIONKEY return NSF20FunctionKey;
    case XK_F21:
      XGPS_FUNCTIONKEY return NSF21FunctionKey;
    case XK_F22:
      XGPS_FUNCTIONKEY return NSF22FunctionKey;
    case XK_F23:
      XGPS_FUNCTIONKEY return NSF23FunctionKey;
    case XK_F24:
      XGPS_FUNCTIONKEY return NSF24FunctionKey;
    case XK_F25:
      XGPS_FUNCTIONKEY return NSF25FunctionKey;
    case XK_F26:
      XGPS_FUNCTIONKEY return NSF26FunctionKey;
    case XK_F27:
      XGPS_FUNCTIONKEY return NSF27FunctionKey;
    case XK_F28:
      XGPS_FUNCTIONKEY return NSF28FunctionKey;
    case XK_F29:
      XGPS_FUNCTIONKEY return NSF29FunctionKey;
    case XK_F30:
      XGPS_FUNCTIONKEY return NSF30FunctionKey;
    case XK_F31:
      XGPS_FUNCTIONKEY return NSF31FunctionKey;
    case XK_F32:
      XGPS_FUNCTIONKEY return NSF32FunctionKey;
    case XK_F33:
      XGPS_FUNCTIONKEY return NSF33FunctionKey;
    case XK_F34:
      XGPS_FUNCTIONKEY return NSF34FunctionKey;
    case XK_F35:
      XGPS_FUNCTIONKEY return NSF35FunctionKey;
    case XK_Delete:
      XGPS_FUNCTIONKEY return NSDeleteFunctionKey;
    case XK_Home:
      XGPS_FUNCTIONKEY return NSHomeFunctionKey;
    case XK_Left:
      XGPS_FUNCTIONKEY return NSLeftArrowFunctionKey;
    case XK_Right:
      XGPS_FUNCTIONKEY return NSRightArrowFunctionKey;
    case XK_Up:
      XGPS_FUNCTIONKEY return NSUpArrowFunctionKey;
    case XK_Down:
      XGPS_FUNCTIONKEY return NSDownArrowFunctionKey;
      //  case XK_Prior:        XGPS_FUNCTIONKEY return NSPrevFunctionKey;
      //  case XK_Next:         XGPS_FUNCTIONKEY return NSNextFunctionKey;
    case XK_End:
      XGPS_FUNCTIONKEY return NSEndFunctionKey;
    case XK_Begin:
      XGPS_FUNCTIONKEY return NSBeginFunctionKey;
    case XK_Select:
      XGPS_FUNCTIONKEY return NSSelectFunctionKey;
    case XK_Print:
      XGPS_FUNCTIONKEY return NSPrintFunctionKey;
    case XK_Execute:
      XGPS_FUNCTIONKEY return NSExecuteFunctionKey;
    case XK_Insert:
      XGPS_FUNCTIONKEY return NSInsertFunctionKey;
    case XK_Undo:
      XGPS_FUNCTIONKEY return NSUndoFunctionKey;
    case XK_Redo:
      XGPS_FUNCTIONKEY return NSRedoFunctionKey;
    case XK_Menu:
      XGPS_FUNCTIONKEY return NSMenuFunctionKey;
    case XK_Find:
      XGPS_FUNCTIONKEY return NSFindFunctionKey;
    case XK_Help:
      XGPS_FUNCTIONKEY return NSHelpFunctionKey;
    case XK_Break:
      XGPS_FUNCTIONKEY return NSBreakFunctionKey;
    case XK_Mode_switch:
      XGPS_FUNCTIONKEY return NSModeSwitchFunctionKey;
    case XK_Scroll_Lock:
      XGPS_FUNCTIONKEY return NSScrollLockFunctionKey;
    case XK_Pause:
      XGPS_FUNCTIONKEY return NSPauseFunctionKey;
    case XK_Clear:
      XGPS_FUNCTIONKEY return NSClearDisplayFunctionKey;
#ifndef NeXT
    case XK_Page_Up:
      XGPS_FUNCTIONKEY return NSPageUpFunctionKey;
    case XK_Page_Down:
      XGPS_FUNCTIONKEY return NSPageDownFunctionKey;
    case XK_Sys_Req:
      XGPS_FUNCTIONKEY return NSSysReqFunctionKey;
#endif
    case XK_KP_F1:
      XGPS_FUNCTIONKEY return NSF1FunctionKey;
    case XK_KP_F2:
...
- (NSPoint)mouseLocationOnScreen:(int)screen window:(int *)win
{
  ...
  ok = XQueryPointer(dpy, [self xDisplayRootWindow], &rootWin, &childWin, &currentX, &currentY,
                     &winX, &winY, &mask);
  p = NSMakePoint(-1, -1);
...
{
  [self receivedEvent:0 type:0 extra:0 forMode:nil];
  return [super getEventMatchingMask:mask beforeDate:limit inMode:mode dequeue:flag];
}

- (void)discardEventsMatchingMask:(unsigned)mask beforeEvent:(NSEvent *)limit
{
  [self receivedEvent:0 type:0 extra:0 forMode:nil];
  [super discardEventsMatchingMask:mask beforeEvent:limit];
}
...
{
  if (generic.lastTimeStamp == 0 || generic.baseXServerTime + MAX_TIME_DIFF * 1000 < last) {
...
- (Time)lastTime
{
  ...
  if (generic.lastTimeStamp == 0 ||
      ((generic.lastTimeStamp + OUT_DATE_TIME_DIFF) < [NSDate timeIntervalSinceReferenceDate])) {
    return CurrentTime;
  } else {
